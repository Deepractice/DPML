import { parse, process, Element, Content, Warning, Node } from '../../../packages/core';
...
async function parseFile(): Promise<void> {
  try {
    ...
    console.log(`- 子元素数量: ${(ast.children[0] as Element).children.length}`);
    ...
    // 显示role信息
    const roleElement = promptElement.children.find((child: Node) => 
      child.type === 'element' && (child as Element).tagName === 'role') as Element | undefined;
    if (roleElement) {
    ...
    // 显示context信息
    const contextElement = promptElement.children.find((child: Node) => 
      child.type === 'element' && (child as Element).tagName === 'context') as Element | undefined;
    if (contextElement) {
      console.log('\n上下文:');
    ...
    // 显示代码信息
    const thinkingElement = promptElement.children.find((child: Node) => 
      child.type === 'element' && (child as Element).tagName === 'thinking') as Element | undefined;
    if (thinkingElement) {
      console.log('\n思考过程:');
      ...
      // 提取代码
      const codeElement = thinkingElement.children.find((child: Node) => 
        child.type === 'element' && (child as Element).tagName === 'code') as Element | undefined;
      if (codeElement) {
