...
async function basicProcessing(): Promise<void> {
  try {
    ...
    if (
      ...
      ast.children[0].type === NodeType.ELEMENT
    ) {
    ...
    try {
      ...
      const metadataElement = promptElement.children.find(
        (child: Node) =>
          ...
          (child as Element).tagName === 'metadata'
      ...
      if (metadataElement) {
        ...
        const titleElement = metadataElement.children.find(
          (child: Node) =>
            child.type === NodeType.ELEMENT &&
            (child as Element).tagName === 'title'
        ) as Element | undefined;
        ...
        const authorElement = metadataElement.children.find(
          (child: Node) =>
            child.type === NodeType.ELEMENT &&
            (child as Element).tagName === 'author'
        ) as Element | undefined;
      ...
      const roleElement = promptElement.children.find(
        (child: Node) =>
          child.type === NodeType.ELEMENT &&
          (child as Element).tagName === 'role'
      ) as Element | undefined;

      if (roleElement) {
        console.log(`角色名称: ${roleElement.attributes.name || '未指定'}`);
        console.log(
          `角色专长: ${roleElement.attributes.expertise || '未指定'}`
        );
      ...
      const contextElement = promptElement.children.find(
        (child: Node) =>
          child.type === NodeType.ELEMENT &&
          (child as Element).tagName === 'context'
      ) as Element | undefined;
      ...
      const thinkingElement = promptElement.children.find(
        (child: Node) =>
          child.type === NodeType.ELEMENT &&
          (child as Element).tagName === 'thinking'
      ) as Element | undefined;
      ...
      const executingElement = promptElement.children.find(
        (child: Node) =>
          child.type === NodeType.ELEMENT &&
          (child as Element).tagName === 'executing'
      ) as Element | undefined;
      ...
      if (executingElement) {
        ...
        if (stepElements.length > 0) {
          stepElements.forEach((step: Element, index: number) => {
            ...
            console.log(
              `步骤 ${index + 1} (${step.attributes.id || '无ID'}): ${stepContent}`
            );
          });
        } else {
      ...
      const referencesElement = promptElement.children.find(
        (child: Node) =>
          child.type === NodeType.ELEMENT &&
          (child as Element).tagName === 'references'
      ) as Element | undefined;
      ...
      if (referencesElement) {
        const referenceElements = referencesElement.children.filter(
          (child: Node) =>
            child.type === NodeType.ELEMENT &&
            (child as Element).tagName === 'reference'
        ) as Element[];

        if (referenceElements.length > 0) {
          referenceElements.forEach((reference: Element) => {
            console.log(
              `- ${reference.attributes.id || '无ID'}: ${reference.attributes.url || '无URL'}`
            );
          });
        } else {
...
async function processingWithOptions(): Promise<void> {
  try {
    ...
    try {
      ...
      if (
        processedDoc &&
        processedDoc.children &&
        processedDoc.children.length > 0
      ) {
...
async function inheritanceExample(): Promise<void> {
  try {
    ...
    const extendedSampleContent = await fs.readFile(
      extendedSamplePath,
      'utf-8'
    );
    ...
    const baseRole = basePrompt.children.find(
      (child: Node) =>
        child.type === NodeType.ELEMENT && (child as Element).tagName === 'role'
    ) as Element;
    ...
    if (baseRole && extendedRole) {
      ...
      console.log(
        `- 内容: ${baseRole.children[0].type === NodeType.CONTENT ? (baseRole.children[0] as Content).value.trim() : '无内容'}`
      );
    ...
    if (baseContext && extendedContext) {
      ...
      console.log(
        `- 内容: ${baseContext.children[0].type === NodeType.CONTENT ? (baseContext.children[0] as Content).value.trim() : '无内容'}`
      );
    ...
    if (baseExecuting && extendedExecuting) {
      ...
      baseSteps.forEach((step: Element, index: number) => {
        ...
        console.log(
          `- 步骤 ${index + 1} (${step.attributes.id}): ${stepContent}`
        );
      });
    ...
    if (baseThinking && extendedThinking) {
      ...
      console.log('\n扩展思考过程:');
      console.log(
        `- 使用继承: ${extendedThinking.attributes.extends ? '是' : '否'}`
      );
