...
DPML 是一种**三方 Prompt 协议**，使用结构化信息让计算机、AI 和人类在 AI 系统中协同工作：

- **面向计算机的 Prompt**：属性驱动配置和执行
- **面向 AI 的 Prompt**：内容驱动理解和推理
- **面向人类的 Prompt**：结构实现观察和控制

传统 Prompt 只服务 AI。DPML 认识到现代 AI 系统需要三种不同类型的 Prompt——给机器的指令、给 AI 的上下文、给人类的可见性——并将它们统一到单一文档中，作为系统协同的基础。

XML 的 tag（概念定义）、attribute（配置数据）、content（语义载体）和 DOM 结构（可视化渲染）四个维度，使其成为唯一能够同时服务计算机、AI 和人类的格式，而 AI 在其中扮演着连接人类意图与计算机执行的关键介质角色。
...
**DPML 通过认识到这三种需求本质上都是"驱动信号"**——引导并驱动系统行为的结构化信息——并将它们统一为单一流转载体来解决这个问题。DPML 在三方之间流转，每一步都驱动下一个角色采取行动，且整个流转过程全程可观测。

### 1.2 目标

DPML 协议旨在：

1. **统一三方驱动**：使计算机、AI 和人类能够通过单一信息载体在 AI 系统中相互驱动
2. **利用结构化信息**：使用 XML 的多维语义（tag/attribute/content/structure）来分离关注点
3. **实现全程可观测**：支持 DPML 流转过程的实时可视化和审计追踪
...
在 AI 系统中，计算机、AI 和人类各自占据独特的生态位，具有不可替代的核心价值：
...
| 维度 | 能力 | 价值 |
|-----|------|------|
| **实践能力** | 通过实践获取经验，总结新知识 | 从具体经验中抽象出规律 |
| **自驱动意识** | 主动发起并参与实践，设定目标 | 定义"做什么"和"为什么做" |
| **创新能力** | 实践与意识结合产生创造力 | 突破现有边界，创造新价值 |

**生态位**：人类是唯一能够主动发起实践、从实践中学习并产生真正创新的角色。
...
| 维度 | 能力 | 价值 |
|-----|------|------|
| **精确执行** | 按指令精确操作不出错，始终如一 | 确保结果的可靠性和一致性 |
| **高速处理** | 计算和数据处理速度远超人类 | 在短时间内完成大量工作 |
| **执行效率** | 精确与速度结合带来极高效率 | 解放人类时间，专注高价值工作 |

**生态位**：计算机是唯一能够以超高速度和绝对精确度执行确定性任务的角色。
...
| 维度 | 能力 | 价值 |
|-----|------|------|
| **模式识别** | 从海量数据中识别复杂模式 | 发现人类难以察觉的规律 |
| **知识整合** | 关联和组织各领域知识 | 提供跨领域的知识连接 |
| **映射能力** | 将抽象意图转化为具体指令 | 连接人类思维与计算机执行 |

**生态位**：AI 是唯一能够理解人类自然语言意图并将其转换为计算机可执行指令的角色。
...
**核心洞察**：在未来的三方交互环境中，人类是终极的创新载体，计算机是终极的效率载体，而 AI 在这两者之间完成转化任务。DPML 正是这种三方协同的实现协议。
...
1. **维度不是分割，而是共享**
   - 不是"这个维度给计算机，那个维度给 AI"
   - 而是"每个维度都提供三方都能理解的语义"

2. **维度数量决定表达能力**
   - 2维（键值对）：只能表达"名称-数据"关系
   - 4维（XML）：可以表达"概念-配置-内容-层级"多重语义

3. **结构化不是开销，是协同基础**
   - 更多维度 = 更丰富的共享语义空间
   - XML 的 4 个维度刚好满足三方协同的最小需求

### 2.3 DPML 如何实现三方驱动

单个 DPML 文档通过 XML 的四个维度，让每个维度都能被三方理解和使用，从而实现三方协同驱动：
...
| 元素分析 | 计算机理解 | AI 理解 | 人类理解 |
|---------|----------|---------|---------|
| **Tag: `<agent>`** | 节点类型，遍历起点 | 这是一个 AI 代理定义 | 看到"代理"概念块 |
| **Tag: `<llm>`** | 子节点，需解析属性 | 这是模型配置信息 | 看到"模型"配置区 |
| **Tag: `<prompt>`** | 文本内容节点 | 这是给我的指令 | 看到"提示词"内容 |
| **Tag: `<metadata>`** | 元数据节点，可选 | 这是系统背景信息 | 看到"元信息"说明 |
| **Attribute: `model="gpt-4"`** | 字符串值 "gpt-4" | 要使用 GPT-4 模型 | 模型是 GPT-4 |
| **Attribute: `temperature="0.7"`** | 浮点数 0.7 | 响应会有一定创造性 | 温度参数是 0.7 |
| **Attribute: `purpose="..."`** | 字符串值 | 系统用途是旅行规划 | 用途：旅行规划助手 |
| **Content: "你是..."** | 纯文本数据 | **我的角色定义**（主场） | AI 的角色描述 |
| **Structure: 嵌套层级** | DOM 树路径 | 上下文包含关系 | **可视化结构**（主场） |

#### 处理流程：三方并行理解

同一个 DPML 文档，三方同时从不同角度理解：

**计算机视角**：
1. 解析 `<agent>` 节点，创建代理对象
2. 读取 `model="gpt-4"`，初始化 GPT-4 客户端
3. 读取 `temperature="0.7"`，设置参数
4. 提取 `<prompt>` 的文本内容，传递给 AI
5. 构建 DOM 树，准备渲染可视化界面

**AI 视角**：
1. 看到 `<agent>`，理解这是代理定义
2. 看到 `<llm>` 及其属性，知道要用 GPT-4 且有一定创造性
3. **重点关注** `<prompt>` 内容："你是专业的旅行规划助手..."
4. 看到 `<metadata>`，了解系统背景和版本信息
5. 基于所有信息理解自己的角色和上下文

**人类视角**：
1. 看到 `<agent>` 结构，知道这是一个代理配置
2. 看到 `model="gpt-4"`，了解使用的模型
3. 看到 `<prompt>` 内容，了解 AI 的角色定位
4. 看到 `<metadata>`，了解系统用途、版本、审核信息
5. **重点关注** DOM 结构的可视化，方便审计和调试

#### 关键洞察

**不是"层叠"，而是"共享"**：

- 错误理解：属性只给计算机、内容只给 AI、结构只给人类
- 正确理解：每个维度都是三方共享的，只是各有侧重

**侧重 ≠ 专属**：

- **Tag**：三方都能理解概念，只是计算机侧重类型，AI 侧重语义，人类侧重结构
- **Attribute**：三方都能读取，只是计算机侧重解析，AI 侧重理解配置含义，人类侧重参数说明
- **Content**：三方都能访问，只是 AI 侧重自然语言内容，计算机侧重数据提取，人类侧重内容阅读
- **Structure**：三方都能利用，只是人类侧重可视化，计算机侧重遍历，AI 侧重上下文层级

这种**多维共享机制**是 DPML 能够统一三方驱动的根本原因。

### 2.4 DPML 作为三方相互驱动的协议

DPML 的本质不仅是信息载体，更是**驱动协议**——它在计算机、AI 和人类之间流转，每一次流转都驱动下一个角色采取行动。

#### DPML 如何驱动三方

| 被驱动方 | DPML 如何驱动 | 驱动结果 |
|---------|-------------|---------|
| **计算机** | 属性配置、工具调用指令 | 执行操作、返回结果、渲染界面 |
| **AI** | Prompt 内容、上下文结构 | 理解意图、生成响应、调用工具 |
| **人类** | 可视化渲染、状态展示 | 观察流程、审计系统、调整意图 |

**关键洞察**：DPML 不是静态文档，而是动态流转的"驱动信号"。

#### 完整的驱动循环

以一个实际场景为例，展示 DPML 如何在三方之间流转并驱动整个系统：

```
1. 人类表达意图（自然语言）
   "帮我查询今天北京的天气"

2. 计算机结构化为 DPML Prompt
   <prompt context="weather-query">
     帮我查询今天北京的天气
   </prompt>
   ↓ (驱动 AI)

3. AI 理解并生成 DPML Tool Call
...
4. 计算机执行工具并返回 DPML Result
   <tool-result name="get-weather" status="success">
     <data type="json">
       {"city": "北京", "temp": 18, "condition": "晴"}
     </data>
   </tool-result>
   ↓ (驱动 AI)

5. AI 基于结果生成 DPML Response
   <response>
     今天北京天气晴朗，温度18°C，适合外出。
   </response>
   ↓ (驱动计算机)
...
DPML 之所以能驱动三方，依赖于 XML 的四个维度，且**每个维度都服务所有三方**，而非简单的一对一映射：

| 维度 | 计算机视角 | AI 视角 | 人类视角 |
|------|-----------|---------|---------|
| **Tag** | Element 节点类型 | 概念定义（`<prompt>` = 提示词） | 语义块结构（可见的功能单元） |
| **Attribute** | 键值对数据结构 | 可理解的配置信息 | 参数说明（辅助理解） |
| **Content** | 文本数据或子节点 | **主要语义场**（自然语言、代码） | 具体内容（根据 type 理解） |
| **Structure** | DOM 树遍历路径 | 上下文层级关系 | **可视化渲染基础** |
...
<!-- type="markdown": 驱动人类和 AI 理解 -->
<description type="markdown">
  ## 功能说明
  这是一个**计算器工具**，支持基本运算。
</description>

<!-- type="prompt": 驱动 AI 推理 -->
<prompt>
  你是一个专业的数学助手，擅长解释复杂的数学概念。
</prompt>

<!-- type="json": 驱动计算机和 AI 处理结构化数据 -->
<data type="json">
...
1. **人类 → AI → 计算机**
   ...
   **价值**：如果没有 AI，人类需要手写完整的 DPML（技术门槛高）

2. **计算机 → AI → 人类**
   - 计算机返回结构化数据（如 JSON、XML）
   - AI 理解数据并生成自然语言响应（DPML Response）
   - 人类看到友好的解释和可视化界面

   **价值**：如果没有 AI，计算机返回的原始数据难以被人类直接理解

3. **DPML 串联整个流程**
   - 传统 Markdown Prompt 无法驱动计算机（缺少结构化配置）
   - 传统 JSON 配置无法驱动 AI（缺少自然语言空间）
   - DPML 统一了两者，让信息在三方之间无损流转

#### 核心价值：可观测的驱动流

DPML 最大的创新是让**整个驱动过程可观测**：
...
DPML 系统：
人类 → DPML Prompt → AI → DPML Tool Call → 计算机 → DPML Result → AI → DPML Response → 人类
优势：每一步都是 DPML，计算机可以将所有 DPML 实时可视化，人类全程可观测
```

**核心价值**：DPML 不仅仅是一种文件格式，而是一个使三方能够相互驱动、信息流转全程可观测的协议基础设施。
...
1. 现代 AI 系统需要**三种类型的驱动**（驱动计算机、驱动 AI、驱动人类）
2. 这些驱动信息必须**统一为单一流转载体**（不是分散在多个文件中）
3. 信息必须**结构性分离**（属性 ≠ 内容 ≠ 结构），才能服务不同角色
4. 驱动过程必须**全程可观测**（每一步 DPML 都可以被实时可视化）
5. AI 在其中扮演**关键介质**（连接人类意图与计算机执行）

**只有 XML 的多维语义能够满足所有五个要求。**

**总结**：DPML 是一个使计算机、AI 和人类能够相互驱动、信息流转全程可观测的三方协同协议。
