...
describe('嵌套上下文结构支持', () => {
  ...
  describe('嵌套上下文的创建与访问', () => {
    it('应该能创建嵌套上下文结构', () => {
      ...
      // 创建第一级嵌套上下文
      const level1Context = contextManager.createNestedContext(
        rootContext,
        'section',
        { sectionTitle: '第一章' }
      );
    ...
    it('应该能在嵌套上下文中创建多级嵌套', () => {
      ...
      // 创建第二级嵌套上下文
      const level2Context = contextManager.createNestedContext(
        level1Context,
        'paragraph',
      ...
      // 创建第三级嵌套上下文
      const level3Context = contextManager.createNestedContext(
        level2Context,
        'sentence',
    ...
    it('应该能从嵌套上下文访问祖先上下文', () => {
      ...
      // 通过嵌套结构获取祖先上下文
      expect(contextManager.getAncestorContext(level3Context, 0)).toBe(
        level3Context
      );
      expect(contextManager.getAncestorContext(level3Context, 1)).toBe(
        level2Context
      );
      expect(contextManager.getAncestorContext(level3Context, 2)).toBe(
        level1Context
      );
      expect(contextManager.getAncestorContext(level3Context, 3)).toBe(
        rootContext
      );
      expect(
        contextManager.getAncestorContext(level3Context, 4)
      ).toBeUndefined();

      // 通过类型获取祖先上下文
      expect(
        contextManager.getAncestorContextByPathElement(
          level3Context,
          'paragraph'
        )
      ).toBe(level2Context);
      expect(
        contextManager.getAncestorContextByPathElement(level3Context, 'section')
      ).toBe(level1Context);
      expect(
        contextManager.getAncestorContextByPathElement(
          level3Context,
  ...
  describe('嵌套上下文的状态传递', () => {
    it('应该能在嵌套上下文之间传递状态', () => {
      ...
      const updatedRoot = contextManager.setVariable(
        rootContext,
        'rootVar',
        'rootValue'
      );
      ...
      const updatedLevel1 = contextManager.setVariable(
        level1Context,
        'level1Var',
        'level1Value'
      );
      ...
      const updatedLevel2 = contextManager.setVariable(
        level2Context,
        'level2Var',
        'level2Value'
      );
      ...
      // 状态应该在各自上下文中可用
      expect(contextManager.getVariable(updatedRoot, 'rootVar')).toBe(
        'rootValue'
      );
      expect(contextManager.getVariable(updatedLevel1, 'level1Var')).toBe(
        'level1Value'
      );
      expect(contextManager.getVariable(updatedLevel2, 'level2Var')).toBe(
        'level2Value'
      );

      // 子上下文应该能访问父上下文的状态
      expect(contextManager.getVariable(updatedLevel2, 'rootVar')).toBe(
        'rootValue'
      );
      expect(contextManager.getVariable(updatedLevel2, 'level1Var')).toBe(
        'level1Value'
      );

      // 父上下文不应该访问到子上下文的状态
      expect(
        contextManager.getVariable(updatedRoot, 'level1Var')
      ).toBeUndefined();
      expect(
        contextManager.getVariable(updatedLevel1, 'level2Var')
      ).toBeUndefined();
    });
    ...
    it('应该能设置嵌套上下文的新变量', () => {
      ...
      const newLevel1Context = contextManager.createNestedContext(
        updatedRoot,
        'section2'
      );

      expect(contextManager.getVariable(newLevel1Context, 'parentVar')).toBe(
        'parentValue'
      );
      ...
      expect(contextManager.getVariable(updatedLevel1, 'parentVar')).toBe(
        'overrideValue'
      );

      // 父上下文的变量不受影响
      expect(contextManager.getVariable(updatedRoot, 'parentVar')).toBe(
        'parentValue'
      );
    });
    ...
    it('应该能创建共享父上下文变量的子上下文', () => {
      ...
      // 创建子上下文后应该能访问父变量
      const childContext = contextManager.createNestedContext(
        updatedRoot,
        'child'
      );

      expect(contextManager.getVariable(childContext, 'sharedVar')).toBe(
        'rootValue'
      );

      // 更新父上下文变量
      const newRootContext = contextManager.setVariable(
        updatedRoot,
        'sharedVar',
        'newValue'
      );
      ...
      const newChildContext = contextManager.createNestedContext(
        newRootContext,
        'newChild'
      );

      expect(contextManager.getVariable(newChildContext, 'sharedVar')).toBe(
        'newValue'
      );
    });
  });
  ...
  describe('嵌套上下文的结果处理', () => {
    it('应该能在嵌套上下文中组合父结果', () => {
      ...
      // 基于更新后的根上下文创建新的子上下文
      const newLevel1 = contextManager.createNestedContext(
        updatedRoot,
        'section'
      );
