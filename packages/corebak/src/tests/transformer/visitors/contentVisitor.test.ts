...
import { ContentVisitor } from '../../../transformer/visitors/contentVisitor';
...
import type { ContentFormat } from '../../../transformer/visitors/contentTypes';
import type { Content } from '../../../types/node';
...
describe('ContentVisitor', () => {
  ...
  it('应该处理包含Markdown格式的内容', async () => {
    ...
    // 斜体格式
    const italicFormat = formats.find(
      (f: ContentFormat) => f.type === 'italic'
    );
  ...
  it('应该处理包含链接的内容', async () => {
    ...
    const content: Content = {
      type: NodeType.CONTENT,
      value:
        '这是一个[链接](https://example.com)和一个[另一个链接](https://test.com "测试")',
      position: {
        start: { line: 1, column: 1, offset: 0 },
