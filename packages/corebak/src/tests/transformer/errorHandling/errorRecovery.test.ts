...
import type { TransformerOptions } from '../../../transformer/interfaces/transformerOptions';
...
describe('错误恢复和自愈机制', () => {
  ...
  it('应支持访问者自动恢复机制的配置', () => {
    ...
    const autoCalls = consoleWarnSpy.mock.calls.filter(
      (call: any[]) =>
        call[0] && typeof call[0] === 'string' && call[0].includes('[自动恢复]')
    );
