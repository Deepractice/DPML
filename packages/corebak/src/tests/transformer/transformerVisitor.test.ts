...
describe('TransformerVisitor', () => {
  ...
  it('应该能创建具有完整访问方法的访问者', () => {
    ...
    // 测试各方法的返回值
    expect(visitor.visitDocument!(mockDocument, mockContext)).toEqual({
      type: 'document',
    });
    expect(visitor.visitElement!(mockElement, mockContext)).toEqual({
      type: 'element',
      name: 'test',
    });
    expect(visitor.visitContent!(mockContent, mockContext)).toEqual({
      type: 'content',
      value: 'Test content',
    });
    expect(visitor.visitReference!(mockReference, mockContext)).toEqual({
      type: 'reference',
      path: 'reference/path',
    });
  });
  ...
  it('应该能创建只关注特定节点类型的访问者', () => {
    ...
    expect(elementVisitor.visitElement!(mockElement, mockContext)).toEqual({
      type: 'element',
      name: 'test',
    });
  });
  ...
  it('应该能创建不指定优先级的访问者(使用默认优先级)', () => {
    ...
    expect(contentVisitor.visitContent!(mockContent, mockContext)).toEqual({
      type: 'content',
      value: 'Test content',
    });
  });
  ...
  it('应该能创建修改上下文的访问者', () => {
    ...
    expect(
      contextModifyingVisitor.visitDocument!(mockDocument, context)
    ).toEqual({ type: 'document', modified: true });
