...
import { DpmlAdapter } from '../../parser/dpml-adapter';
...
import { NodeType, isElement, isContent } from '../../types/node';
...
describe('DPMLAdapter', () => {
  ...
  describe('Element处理', () => {
    it('应该正确保留元素的属性', async () => {
      ...
      expect(promptElement.attributes).toEqual(
  ...
  describe('Content处理', () => {
    ...
    it('应该正确处理混合内容', async () => {
      ...
      const emphasisElement = promptElement.children.find(
        child =>
          child.type === NodeType.ELEMENT &&
          (child as Element).tagName === 'emphasis'
      ) as Element;
      ...
      expect((emphasisElement.children[0] as Content).value).toBe(
        '这是强调的文本'
      );
    });
  });
  ...
  describe('根标签处理', () => {
    it('应该正确处理任意根标签的XML', async () => {
      ...
      const testCases = [
        ...
        {
          xml: '<custom-root>自定义内容</custom-root>',
          expectedTagName: 'custom-root',
        },
        {
          xml: '<workflow id="test">工作流内容</workflow>',
          expectedTagName: 'workflow',
        },
      ];
    ...
    it('应该确保Document总是有子节点', async () => {
      ...
      const nestedElement = rootElement.children.find(
        child => isElement(child) && (child as Element).tagName === 'nested'
      ) as Element;
