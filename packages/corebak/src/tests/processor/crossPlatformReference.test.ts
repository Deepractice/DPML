...
import { PlatformType } from '../../utils/pathUtils';
  ...
  return {
    promises: {
      readFile: vi.fn(async path => {
        // 模拟文件内容
        if (
          path.includes('windows-file.xml') ||
          path.includes('windows-file')
        ) {
          return `<root>
...
describe('跨平台文件引用处理', () => {
  ...
  let originalConvertPathForPlatform: typeof pathUtils.convertPathForPlatform;
  ...
  beforeEach(() => {
    ...
    referenceResolver.registerProtocolHandler(
      'file',
  ...
  it('应该处理Windows格式的文件路径引用', async () => {
    ...
    vi.spyOn(pathUtils, 'convertPathForPlatform').mockImplementation(
      (path, platform) => {
    ...
    const findWindowsContent = (element: Element) => {
      ...
      if (element.children) {
        for (const child of element.children) {
          if (child.type === NodeType.ELEMENT) {
            findWindowsContent(child as Element);
          }
        }
      }
    };
  ...
  it('应该处理Unix格式的文件路径引用', async () => {
    ...
    const findUnixContent = (element: Element) => {
      ...
      if (element.children) {
        for (const child of element.children) {
          if (child.type === NodeType.ELEMENT) {
            findUnixContent(child as Element);
          }
        }
      }
    };
