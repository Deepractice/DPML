...
import type { ProcessingContext } from '../../../processor/processingContext';
...
describe('访问者扩展能力测试', () => {
  ...
  it('应该能注册和执行自定义访问者', async () => {
    ...
    expect(resultElement.attributes['data-processed']).toBe(
      'by-custom-visitor'
    );
  ...
  it('应该根据优先级排序执行访问者', async () => {
    ...
    // 验证调用顺序
    expect(highSpy.mock.invocationCallOrder[0]).toBeLessThan(
      mediumSpy.mock.invocationCallOrder[0]
    );
    expect(mediumSpy.mock.invocationCallOrder[0]).toBeLessThan(
      lowSpy.mock.invocationCallOrder[0]
    );
  });
  ...
  it('应该支持多访问者协作处理', async () => {
    ...
    expect(resultElement.attributes['data-also-processed']).toBe(
      'by-another-visitor'
    );
  });
