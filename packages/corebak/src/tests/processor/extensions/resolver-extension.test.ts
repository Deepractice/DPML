...
import { NodeType, Content } from '../../../types/node';

import type { ProtocolHandler } from '../../../processor/interfaces/protocolHandler';
import type { ReferenceResolver } from '../../../processor/interfaces/referenceResolver';
import type { Element, Document, Reference } from '../../../types/node';
...
class TransformingReferenceResolver implements ReferenceResolver {
  ...
  private transformFunctions: Record<
    string,
  ...
  addTransform(
    protocol: string,
    transform: (reference: Reference) => Reference
  ): void {
  ...
  async resolve(
    ...
    if (this.transformFunctions[reference.protocol]) {
      transformedReference =
        this.transformFunctions[reference.protocol](reference);
    }
...
class CustomReferenceResolver implements ReferenceResolver {
  ...
  async resolve(
    ...
    try {
      ...
      throw new Error(
        `引用解析失败: ${reference.protocol}:${reference.path}: ${(error as Error).message}`
      );
    }
  }
}
...
describe('引用解析器扩展能力测试', () => {
  ...
  it('应该支持自定义的引用转换解析器', async () => {
    // 创建转换解析器
    const transformingResolver = new TransformingReferenceResolver(
      defaultResolver
    );
