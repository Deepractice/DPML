...
import { ReferenceError } from '../../../errors/types';
...
import type { Reference, Element, Document } from '../../../types/node';
...
describe('IdProtocolHandler', () => {
  ...
  it('应该在没有设置上下文时抛出错误', async () => {
    ...
    await expect(handler.handle(reference)).rejects.toThrow(
      'ID协议处理器未设置上下文'
    );
  });
