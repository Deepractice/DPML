...
import type { ProtocolHandler } from '../../processor/interfaces';
import type { Reference, Document } from '../../types/node';
...
describe('DefaultReferenceResolver', () => {
  ...
  it('处理不支持的协议时应抛出错误', async () => {
    ...
    await expect(resolver.resolve(unknownReference, context)).rejects.toThrow(
      ReferenceError
    );
    await expect(resolver.resolve(unknownReference, context)).rejects.toThrow(
      '不支持的引用协议'
  ...
  it('处理引用解析失败时应抛出错误', async () => {
    ...
    await expect(
      failingResolver.resolve(httpReference, context)
    ).rejects.toThrow(ReferenceError);
    await expect(
      failingResolver.resolve(httpReference, context)
    ).rejects.toThrow('引用解析失败');
  });
});
