...
} from '../../../processor/visitors/attributeValidationVisitor';
import { DocumentMode } from '../../../processor/visitors/documentMetadataVisitor';
...
describe('AttributeValidationVisitor', () => {
  ...
  it('应该验证属性类型', async () => {
    ...
    await expect(
      visitor.visitElement(invalidElement, context)
    ).rejects.toThrow();
  ...
  it('应该验证必需属性', async () => {
    ...
    context.variables.metadata.mode = DocumentMode.STRICT;
    await expect(
      visitor.visitElement(missingRequiredElement, context)
    ).rejects.toThrow();
  ...
  it('应该识别未知属性', async () => {
    ...
    const result2 = await visitor.visitElement(
      extensionAttributeElement,
      context
    );
  ...
  it('应该处理无标签定义的元素', async () => {
    ...
    await expect(
      visitor.visitElement(undefinedTagElement, context)
    ).rejects.toThrow();
  });
});
