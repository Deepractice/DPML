...
describe('复杂功能集成测试', () => {
  ...
  it('应该处理继承+引用组合场景', async () => {
    ...
    const inheritanceVisitor = new InheritanceVisitor(
    ...
    // 手动测试继承处理
    const processedExtended = await inheritanceVisitor.visitElement(
      extendedComponent,
      context
    );
  ...
  it('应该模拟引用处理', async () => {
    ...
    // 手动处理引用节点验证功能
    const processedRef = await referenceVisitor.visitReference(
      referenceNode,
      new ProcessingContext(document, '/test')
    );
    ...
    // 手动处理内容节点验证功能
    const processedContent = await referenceVisitor.visitContent(
      content,
      new ProcessingContext(document, '/test')
    );
