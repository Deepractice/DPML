...
import { Content, Document, Element, Node, NodeType } from '../../types/node';
import { ContentFormat, ContentLink, ContentImage } from './contentTypes';
...
export class ContentVisitor implements TransformerVisitor {
   ...
   */
  async visitDocumentAsync(document: Document, context: TransformContext): Promise<Document> {
    // 递归处理子节点
    if (document.children && document.children.length > 0) {
      for (let i = 0; i < document.children.length; i++) {
        document.children[i] = await this.visitAsync(document.children[i], context);
      }
    }
  ...
  async visitElementAsync(element: Element, context: TransformContext): Promise<Element> {
    ...
    if (element.children && element.children.length > 0) {
      for (let i = 0; i < element.children.length; i++) {
        element.children[i] = await this.visitAsync(element.children[i], context);
      }
    }
  ...
  visitContent(content: Content, context: TransformContext): Content {
    ...
    // 处理自定义格式
    if (this.options.customFormatters && this.options.customFormatters.length > 0) {
   ...
   * @private
   */
  private processMarkdownFormats(content: Content, context: TransformContext): Record<string, any> {
    const value = content.value || '';
    ...
    for (const handler of this.markdownFormatHandlers) {
      ...
      while ((match = pattern.exec(value)) !== null) {
        const result = handler.process(match as unknown as FormatMatch, content, context);
   ...
   * @private
   */
  private processCustomFormats(content: Content, context: TransformContext): ContentFormat[] {
    if (!this.options.customFormatters || this.options.customFormatters.length === 0) {
      return [];
    }
  ...
  private decodeEntities(value: string): { decoded: boolean; value: string } {
    ...
    for (const entity in this.htmlEntities) {
      if (result.includes(entity)) {
        hasEntities = true;
        result = result.replace(new RegExp(entity, 'g'), this.htmlEntities[entity]);
      }
    }
