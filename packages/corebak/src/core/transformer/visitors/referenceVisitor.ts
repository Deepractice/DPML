 ...
 */
export type ReferenceFormatter = (
  reference: Reference,
...
export class ReferenceVisitor implements TransformerVisitor {
   ...
   */
  private async processNestedReferencesAsync(
    content: any,
    context: TransformContext
  ): Promise<any> {
    if (!content || typeof content !== 'object') {
    ...
    if (Array.isArray(content)) {
      return Promise.all(
        content.map(item => this.processNestedReferencesAsync(item, context))
      );
    }

    // 处理对象中的引用数组
    if (content.references && Array.isArray(content.references)) {
      content.references = await Promise.all(
        content.references.map(async (ref: any) => {
          if (ref && ref.type === NodeType.REFERENCE) {
    ...
    for (const key in result) {
      if (
        ...
        result[key] = await this.processNestedReferencesAsync(
          result[key],
          context
        );
      }
  ...
  private async processAsyncData(content: any): Promise<any> {
    ...
    if (asyncKeys.length > 0) {
      const resolvedValues = await Promise.all(
        asyncKeys.map(key => result[key])
      );

      asyncKeys.forEach((key, index) => {
        // 将Promise的解析结果合并到结果对象中
        if (
          typeof resolvedValues[index] === 'object' &&
          resolvedValues[index] !== null
        ) {
   ...
   * @returns 格式化后的引用
   */
  private formatDefaultReference(
    reference: Reference,
    context: TransformContext
  ): any {
    const { resolved } = reference;
