...
import type { Element, Node, Document, Content } from '../../types/node';
...
export class SpecialElementVisitor implements TransformerVisitor {
    ...
    if (this.options.processChildren && element.children) {
      for (let i = 0; i < element.children.length; i++) {
        ...
        if (child.type === NodeType.ELEMENT) {
          element.children[i] = await this.visitElement(
            child as Element,
            context
          );
        }
  ...
  private processLinkElement(
    ...
    if (element.children && element.children.length > 0) {
      ...
      const contentNode = element.children.find(
        child => child.type === NodeType.CONTENT
      ) as Content;
   ...
   * @private
   */
  private processUnorderedListElement(
    element: Element,
    context: TransformContext
  ): Element {
    const meta = element.meta || {};
   ...
   * @private
   */
  private processOrderedListElement(
    element: Element,
    context: TransformContext
  ): Element {
    const meta = element.meta || {};
  ...
  private processVideoElement(
    ...
    const height = parseFloat(element.attributes.height || '0');
    const controls =
      element.attributes.controls === 'true' ||
      element.attributes.controls === '';
  ...
  private extractTableData(tableElement: Element): string[][] {
    ...
    const thead = tableElement.children.find(
      child =>
        child.type === NodeType.ELEMENT &&
        (child as Element).tagName === 'thead'
    ) as Element;

    const tbody = tableElement.children.find(
      child =>
        child.type === NodeType.ELEMENT &&
        (child as Element).tagName === 'tbody'
    ) as Element;
    ...
    if (!thead && !tbody) {
      ...
      const rows = tableElement.children.filter(
        child =>
          child.type === NodeType.ELEMENT && (child as Element).tagName === 'tr'
      ) as Element[];
  ...
  private extractRowData(rowElement: Element): string[] {
    ...
    const cells = rowElement.children.filter(
      child =>
        child.type === NodeType.ELEMENT &&
        ((child as Element).tagName === 'td' ||
          (child as Element).tagName === 'th')
    ) as Element[];
  ...
  private extractListItems(listElement: Element): string[] {
    ...
    const items = listElement.children.filter(
      child =>
        child.type === NodeType.ELEMENT && (child as Element).tagName === 'li'
    ) as Element[];
