...
import type { Content, Reference } from 'packages/corebak/src/types/node';
import type {
  NodeVisitor,
  ProcessingContext,
  ReferenceResolver,
...
export class ReferenceVisitor implements NodeVisitor {
  ...
  private readonly referenceRegex =
    /@((?:[a-zA-Z0-9._\-]+:)?[a-zA-Z0-9._\-\/\?=&%:]+(?:\.[a-zA-Z0-9]+)?(?:#[a-zA-Z0-9_\-]+)?)/g;
    ...
    for (const match of matches) {
      ...
      try {
        ...
        newValue = newValue.replace(
          fullMatch,
          this.formatResolvedValue(resolved.value)
        );
      } catch (error) {
        // 引用解析失败，保留原始引用
