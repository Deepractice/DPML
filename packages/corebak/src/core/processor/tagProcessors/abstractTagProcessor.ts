...
import type { ValidationError, ValidationWarning } from 'packages/corebak/src/errors/types';
...
export abstract class AbstractTagProcessor implements TagProcessor {
  ...
  async process(
    ...
    // 处理特定属性（交由子类实现）
    const specificMetadata = await this.processSpecificAttributes(
      otherAttrs,
      element,
      context
    );
   ...
   */
  protected findFirstChildByTagName(
    element: Element,
    tagName: string
  ): Element | undefined {
    for (const child of element.children) {
   ...
   * @returns 元素数组
   */
  protected findChildrenByTagName(
    element: Element,
    tagName: string
  ): Element[] {
