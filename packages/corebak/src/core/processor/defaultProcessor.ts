...
import { ErrorHandler } from 'packages/corebak/src/core/processor/errors/errorHandler';
import {
  ProcessingError,
  ErrorSeverity,
} from 'packages/corebak/src/core/processor/errors/processingError';
import { ProcessingContext as ProcessingContextImpl } from 'packages/corebak/src/core/processor/processingContext';
import { NodeType } from 'packages/corebak/src/types/node';
...
export class DefaultProcessor implements Processor {
  ...
  async process(document: Document, path: string): Promise<Document> {
    try {
      ...
      // 处理文档
      const processedDocument = await this.visitDocument(
        document,
        this.context
      );
