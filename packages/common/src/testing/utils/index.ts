 ...
 * @returns 测试上下文
 */
export function createTestContext<T extends Record<string, any> = Record<string, any>>() {
 ...
 * @returns 沙盒对象
 */
export function createSandbox<T extends Record<string, any>>(props: T = {} as T) {
  const id = randomUuid()();
  ...
  return {
     ...
     * @param value 属性值
     */
    set<K extends keyof typeof sandbox>(key: K, value: (typeof sandbox)[K]): void {
      sandbox[key] = value;
    },
 ...
 * @returns 记录包装函数
 */
export function recordCalls<T extends (...args: any[]) => any>(fn: T): {
  fn: (...args: Parameters<T>) => ReturnType<T>;
  calls: Array<{ args: Parameters<T>; result: ReturnType<T>; time: number }>;
  reset: () => void;
} {
  const calls: Array<{ args: Parameters<T>; result: ReturnType<T>; time: number }> = [];
