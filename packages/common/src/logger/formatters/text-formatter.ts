...
export class TextFormatter implements LogFormatter {
  ...
  constructor(options: TextFormatterOptions = {}) {
    ...
    this.timestampFormatter = options.timestampFormatter || ((date) => date.toISOString());
    this.template = options.template || '[{timestamp}] [{packageName}] [{level}] {message}';
  }
  ...
  format(level: LogLevel, message: string, meta: LogMeta): string {
    const timestamp = this.showTimestamp ? this.formatTimestamp(meta.timestamp) : '';
