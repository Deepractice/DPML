...
const measurePerformance = async (
  ...
  console.log(
    `性能测试 [${name}]: 平均: ${average.toFixed(3)}ms, 最小: ${min.toFixed(3)}ms, 最大: ${max.toFixed(3)}ms`
  );
...
describe('IT-性能测试', () => {
  describe('日志系统性能', () => {
    test('IT-PERF-001: 日志记录应满足性能要求', () => {
      ...
      console.log(
        `日志性能测试: 平均: ${averageTime.toFixed(3)}ms, 总时间: ${totalTime.toFixed(3)}ms, 数量: 10000`
      );
