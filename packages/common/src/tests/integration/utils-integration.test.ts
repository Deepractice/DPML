...
describe('IT-Utils工具集成测试', () => {
  describe('路径和文件操作', () => {
    test('IT-UTILS-001: 路径操作工具在不同平台应表现一致', () => {
      ...
      // 验证路径有效
      expect(
        utils.path.isAbsolute(normalizedWinPath) ||
          utils.path.isAbsolute(normalizedUnixPath)
      ).toBe(true);
    ...
    test('IT-UTILS-002: 路径操作工具与Node.js内置模块功能一致', () => {
      ...
      expect(utils.path.normalize(utilsResult)).toBe(
        utils.path.normalize(nodeResult)
      );
    });
  });
