    ...
    describe('HTTP客户端模拟测试', () => {
      it('应该支持HTTP请求匹配和响应模拟', async () => {
        ...
        httpClient
          ...
          .reply(200, { users: [{ id: 1, name: 'User 1' }] });
        httpClient
          .onPost('https://api.example.com/users')
          .reply(201, { id: 2, name: 'User 2' });

        // 设置错误响应
        httpClient
          .onGet('https://api.example.com/error')
        ...
        expect(httpClient.history.get[0].url).toBe(
          'https://api.example.com/users'
        );
        expect(httpClient.history.get[1].url).toBe(
          'https://api.example.com/error'
        );
        expect(httpClient.history.post[0].url).toBe(
          'https://api.example.com/users'
        );
      });
    });
