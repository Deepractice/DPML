...
describe('异步测试工具', () => {
  ...
  describe('waitForCondition', () => {
    ...
    it('应该定期检查条件直到满足', async () => {
      ...
      const promise = waitForCondition(condition, { interval: 100 }).then(
        () => {
          promiseResult.done = true;
        }
      );
  ...
  describe('waitForEvent', () => {
    it('应该等待事件触发', async () => {
      ...
      const eventPromise = waitForEvent<{ value: string }, typeof emitter>(
        emitter,
        'test-event'
      );
    ...
    it('使用过滤器时应该只响应匹配的事件', async () => {
      ...
      const eventPromise = waitForEvent<{ id: number }, typeof emitter>(
        emitter,
        'test-event',
        {
