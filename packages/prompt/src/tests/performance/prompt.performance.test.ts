...
Additional custom content
  ...
  const loremIpsum =
    '这是一段用于填充的文本，目的是测试处理器对大型文档的处理能力。我们需要确保系统能够高效地处理大量文本数据，同时保持内存使用合理。这段文本将被重复多次以达到测试所需的文档大小。';
...
describe('Processing Speed Tests (PT-P-002)', () => {
  test('should process 100KB document in less than 500ms', async () => {
    ...
    console.log(
      `Time to process 100KB document: ${processingTime.toFixed(2)}ms`
    );
  });
});
...
describe('Concurrent Processing Tests (PT-P-003)', () => {
  test('should handle multiple concurrent prompt processing efficiently', async () => {
    ...
    const docs = await Promise.all(
      Array(numConcurrent)
        .fill(0)
        .map(() => generateDPMLDocument(50)) // 每个50KB
    );

    // 测量并发处理时间
    const startTime = performance.now();
    const results = await Promise.all(docs.map(doc => generatePrompt(doc)));
    const endTime = performance.now();
...
${Array(20).fill('Custom section 3 content.').join('\n')}
    ...
    expect(result).toBeTruthy();
    console.log(
      `Time to process complex document: ${processingTime.toFixed(2)}ms`
    );
...
describe('Inheritance Processing Tests (PT-P-005)', () => {
  test('should process multi-level inheritance efficiently', async () => {
    ...
    expect(result).toBeTruthy();
    console.log(
      `Time to process inheritance chain: ${processingTime.toFixed(2)}ms`
    );
