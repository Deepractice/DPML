...
import type { FormatTemplates } from '../../transformers/formatConfig';

// 替换这些使用spy而不是mock
vi.spyOn(fs.promises, 'readFile');
vi.spyOn(PromptTransformer.prototype, 'transform').mockReturnValue(
  '模拟转换结果'
);
...
describe('API功能测试', () => {
  ...
  it('配置选项传递 (UT-API-002)', async () => {
    ...
    // 验证配置被传递
    expect(PromptTransformer.prototype.transform).toHaveBeenCalled();
    const transformerInstance = (PromptTransformer.prototype.transform as any)
      .mock.instances[0];
