...
import { generatePrompt, processPrompt, transformPrompt } from '../..';

import type { GeneratePromptOptions } from '../../../api';
...
vi.mock('../..', async () => {
  ...
  const mockComplexPrompt: ProcessedPrompt = {
    ...
    tags: {
      role: {
        content:
          '你是一位资深AI专家，专注于大语言模型研究，有超过10年的行业经验。你的主要研究领域包括机器学习、自然语言处理和强化学习。',
        attributes: {
          type: 'expert',
          domain: 'ai',
          experience: 'senior',
      ...
      context: {
        content:
          '用户正在进行一项关于大语言模型性能评估的研究。他们需要设计一套系统的评估方法，涵盖多种模型能力。',
        attributes: {
          domain: 'research',
          audience: 'technical',
        },
  ...
  return {
    ...
    processPrompt: vi.fn((...args: unknown[]) => {
      ...
      if (
        typeof input === 'string' &&
        (input.includes('child-prompt') || input.includes('parent'))
      ) {
        return Promise.resolve(mockInheritedPrompt);
      } else if (
        typeof input === 'string' &&
        input.includes('complex-prompt')
      ) {
        return Promise.resolve(mockComplexPrompt);
      } else if (
        typeof input === 'string' &&
        input.includes('config-test-prompt')
      ) {
    ...
    transformPrompt: vi.fn((...args: unknown[]) => {
      ...
      if (input === mockInheritedPrompt) {
        ...
        if (
          ...
          options.format.context.title === '## 背景信息'
        ) {
          return '## 背景信息\n背景：基础上下文。\n\n## 角色\n你是一个AI助手。\n\n## 思考过程\n「思考步骤」';
        } else {
    ...
    generatePrompt: vi.fn((...args: unknown[]) => {
      ...
      if (typeof input === 'string' && input.includes('complex-prompt')) {
        ...
        typeof input === 'string' &&
        (input.includes('error-prompt') ||
          input.includes('invalid-nesting') ||
          input.includes('unclosed-tag'))
      ) {
...
describe('高级集成测试', () => {
  ...
  it('复杂提示测试 (IT-P-005)', async () => {
    ...
    expect(processed.tags.context.attributes).toHaveProperty(
      'domain',
      'research'
    );
    expect(processed.tags.thinking.attributes).toHaveProperty(
      'structured',
      'true'
    );
    expect(processed.tags.executing.attributes).toHaveProperty(
      'detail',
      'high'
    );
    expect(processed.tags.testing.attributes).toHaveProperty(
      'thoroughness',
      'comprehensive'
    );
    expect(processed.tags.protocol.attributes).toHaveProperty(
      'interaction',
      'professional'
    );
  ...
  it('错误处理测试 (IT-P-007)', async () => {
    ...
    const validateResult = await generatePrompt(invalidNestingDpml, {
      validateOnly: true,
    });
    ...
    // 4. 未知标签错误处理
    // 宽松模式应该忽略未知标签
    const unknownTagResult = await generatePrompt(unknownTagDpml, {
      strictMode: false,
    });
