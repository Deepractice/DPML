...
import type { ProcessedPrompt } from '../../../types';
...
vi.mock('../..', async () => {
  ...
  // 模拟转换结果
  const mockTransformedText =
    '# 角色\n你是一位专业助手\n\n# 上下文\n用户正在寻求编程问题的解决方案。\n\n# 思考方式\n分析用户问题，提供清晰解答。\n确认理解用户意图。\n\n# 执行步骤\n1. 仔细阅读用户问题\n2. 提供详细解答\n3. 确认是否解决了问题';

  // 模拟完整标签集转换结果
  const mockFullTagsText =
    '# 角色\n专业技术顾问\n\n# 上下文\n用户是软件开发团队，需要架构建议。\n\n# 思考方式\n分析团队需求和技术限制。\n考虑最佳实践和可扩展性。\n\n# 执行步骤\n1. 了解团队当前架构\n2. 识别架构问题\n3. 提出改进建议\n\n# 质量检查\n检查建议是否:\n- 解决了现有问题\n- 考虑了未来扩展\n- 符合行业最佳实践\n\n# 交互协议\n始终使用专业术语。\n提问时必须明确指出问题的优先级。\n\n# 自定义内容\n示例回答:\n"根据您的微服务架构，我建议..."';
  ...
  return {
    ...
    processPrompt: vi.fn((input: unknown) => {
      if (typeof input === 'string' && input.includes('basic-prompt')) {
        ...
        typeof input === 'string' &&
        input.includes('full-tags-prompt')
      ) {
    ...
    transformPrompt: vi.fn((input: unknown) => {
      if (input === mockProcessedPrompt) {
        ...
        input &&
        typeof input === 'object' &&
        'tags' in (input as any) &&
        (input as any).tags &&
        'role' in (input as any).tags
      ) {
    ...
    generatePrompt: vi.fn((input: unknown) => {
      if (typeof input === 'string' && input.includes('basic-prompt')) {
        ...
        typeof input === 'string' &&
        (input.includes('test-prompt') || input.includes('core-integration'))
      ) {
