...
import { JSONOutputAdapter } from '../../src/transformer/adapters/jsonOutputAdapter';
import { XMLOutputAdapter } from '../../src/transformer/adapters/xmlOutputAdapter';
import { MarkdownOutputAdapter } from '../../src/transformer/adapters/markdownOutputAdapter';
...
describe('复杂文档转换集成测试', () => {
  ...
  it('应该能处理深层嵌套的复杂元素结构', async () => {
    ...
    console.log('处理后的文档键:', processedDoc ? Object.keys(processedDoc).join(', ') : '无');
    ...
    console.log('JSON Output 键:', jsonOutput ? Object.keys(jsonOutput).join(', ') : '无');
    
    if (jsonOutput && typeof jsonOutput === 'object') {
      // 输出 JSON 结构的前300个字符
      console.log('JSON Output 内容预览:', 
      ...
      if (jsonOutput.document) {
        ...
        expect(jsonOutput.document.section.container.paragraph).toBe('顶层段落');
  ...
  it('应该能根据不同格式生成不同输出', async () => {
    ...
    const jsonOutput = transformer.transform(processedDoc, { format: 'json' });
    const xmlOutput = transformer.transform(processedDoc, { format: 'xml' });
    const markdownOutput = transformer.transform(processedDoc, { format: 'md' });
