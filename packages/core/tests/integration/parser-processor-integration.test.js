...
describe('Parser与Processor集成测试', () => {
    ...
    it('应该在处理过程中保留Parser解析的结构信息', async () => {
        ...
        // 验证section的子元素结构
        if (section) {
            const hasTitle = section.children.some(child => child.type === NodeType.ELEMENT && child.attributes && child.nodeName === 'title');
            const hasContent = section.children.some(child => child.type === NodeType.ELEMENT && child.attributes && child.nodeName === 'content');
...
//# sourceMappingURL=parser-processor-integration.test.js.map