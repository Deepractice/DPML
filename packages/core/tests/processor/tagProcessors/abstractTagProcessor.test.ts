...
import { Element, NodeType, Content, Node, Document } from '@core/types/node';
import { ProcessingContext } from '@core/processor/interfaces';
...
describe('AbstractTagProcessor', () => {
  ...
  it('应该正确处理验证错误和警告', async () => {
    ...
    expect(result.metadata!.validationWarnings![0].code).toBe('DEPRECATED_ATTR');
  });
  ...
  it('应该查找子元素', () => {
    ...
    // 测试查找第一个子元素
    const firstChild = (processor as any).findFirstChildByTagName(element, 'child1');
    expect(firstChild).toBeDefined();
    expect(firstChild.tagName).toBe('child1');
    expect(firstChild.attributes.id).toBe('first');
    
    // 测试查找所有子元素
    const allChildren = (processor as any).findChildrenByTagName(element, 'child1');
    ...
    // 测试未找到的情况
    const notFound = (processor as any).findFirstChildByTagName(element, 'notexist');
    expect(notFound).toBeUndefined();
    
    const emptyList = (processor as any).findChildrenByTagName(element, 'notexist');
