...
describe('ReferenceVisitor', () => {
    ...
    it('应该在内容中识别和解析引用', async () => {
        ...
        mockReferenceResolver.resolve.mockImplementationOnce((ref) => {
    ...
    it('应该处理引用解析失败的情况', async () => {
        ...
        // 模拟引用解析失败
        mockReferenceResolver.resolve.mockRejectedValue(new DPMLError({
    ...
    it('应该正确匹配内容中复杂的引用格式', async () => {
        ...
        // 模拟所有引用解析
        mockReferenceResolver.resolve.mockImplementation((ref) => {
            return Promise.resolve({
                reference: ref,
        ...
        // 验证内容中的所有引用都被正确替换
        expect(result.value).toBe('复杂引用: http:http://example.com/path/to/resource.dpml-resolved 和 ' +
            'file:./relative/path.dpml-resolved 以及 ' +
            'id:element-123-resolved');
    });
});
//# sourceMappingURL=referenceVisitor.test.js.map