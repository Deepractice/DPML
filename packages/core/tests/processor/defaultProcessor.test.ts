...
import { NodeVisitor, Processor, ProcessingContext, ProtocolHandler, ReferenceResolver } from '../../src/processor/interfaces';
  ...
  describe('节点遍历逻辑', () => {
    it('应该递归遍历所有节点', async () => {
      ...
      expect(trackingVisitor.visitContent).toHaveBeenCalledWith(
        ((nestedDocument.children[0] as Element).children[0] as Element).children[0],
        expect.any(Object)
      );
