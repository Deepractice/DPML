...
import { Reference, NodeType } from '../../../src/types/node';
...
describe('FileProtocolHandler', () => {
  let handler: FileProtocolHandler;
  
  // 创建测试引用对象的辅助函数
  const createReference = (filePath: string): Reference => ({
  ...
  it('应该使用正确的默认选项', () => {
    ...
    // 验证默认工作目录是当前目录
    expect((defaultHandler as any).baseDir).toBe('/default/cwd');
    ...
    expect((customHandler as any).baseDir).toBe('/custom/dir');
  });
  ...
  it('应该处理设置了contextPath的情况', async () => {
    ...
    (contextHandler as any).contextPath = '/context/path';
  ...
  it('应该可以设置上下文路径', () => {
    ...
    expect((contextHandler as any).contextPath).toBe('/new/context/path');
  });
}); 