...
describe('HttpProtocolHandler', () => {
    let handler;
    // 创建测试引用对象的辅助函数
    const createReference = (path, protocol = 'http') => ({
    ...
    it('应该使用正确的默认选项', () => {
        ...
        expect(handler.allowInsecure).toBe(false);
        // 创建自定义选项的处理器
        const customHandler = new HttpProtocolHandler({
            timeout: 5000,
            allowInsecure: true
        });
        expect(customHandler.timeout).toBe(5000);
        expect(customHandler.allowInsecure).toBe(true);
    });
    ...
    it('应该处理网络错误', async () => {
        ...
        fetch.mockRejectedValue(new Error('网络连接失败'));
...
//# sourceMappingURL=httpProtocolHandler.test.js.map