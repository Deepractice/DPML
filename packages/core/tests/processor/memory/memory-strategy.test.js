...
describe('处理器内存策略测试', () => {
    ...
    it('应该使用引用缓存避免重复解析', async () => {
        ...
        // 创建一个简单的协议处理器
        const mockHandler = {
            canHandle: (protocol) => protocol === 'id',
    ...
    it('应该优化深层嵌套文档的处理', async () => {
        // 创建一个深层嵌套的测试文档
        function createNestedDocument(depth) {
            const document = {
                type: NodeType.DOCUMENT,
                children: [],
                position: {
        ...
        // 创建一个会在超过一定深度时失败的函数
        function createStackOverflowFunction(maxDepth) {
            return function recursiveFunction(depth = 0) {
                if (depth >= maxDepth) {
...
//# sourceMappingURL=memory-strategy.test.js.map