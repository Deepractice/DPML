...
describe('内存优化器', () => {
    ...
    it('应该使用节点池优化内存使用', () => {
        ...
        node2.testProp = 'test';
        expect(node1.testProp).toBe('test');
    });
    it('应该通过引用缓存记录和重置', () => {
        // 初始状态下不应该需要清理缓存
    ...
    it('应该正确清理内存', () => {
        ...
        // 添加自定义探测属性
        optimizer.probeProperty = 'memory-test';
        // 清理内存
        optimizer.cleanupMemory();
        // 探测属性应该仍然存在
        expect(optimizer.probeProperty).toBe('memory-test');
    ...
    it('应该支持禁用迭代遍历', () => {
        ...
        // 模拟递归方法
        vi.spyOn(nonIterativeOptimizer, 'traverseNode');
        // 创建一个简单文档
        const document = {
            type: NodeType.DOCUMENT,
            children: [],
        ...
        // 验证递归方法被调用
        expect(nonIterativeOptimizer.traverseNode).toHaveBeenCalledWith(document, mockVisitor);
    });
    it('应该支持配置选项', () => {
        // 创建一个自定义配置的优化器
        ...
        node2.testProp = 'test';
        expect(node1.testProp).toBeUndefined();
        // 创建启用引用缓存的优化器
...
//# sourceMappingURL=memoryOptimizer.test.js.map