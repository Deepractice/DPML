...
describe('基本处理流程', () => {
    ...
    it('应该处理简单文档', async () => {
        ...
        // 处理文档
        const result = await processor.process(simpleDocument, '/test/simple-document.xml');
        // 验证结果
        ...
        expect(result.children[0].attributes.id).toBe('root1');
        // 验证子元素
        ...
        expect(childElement.children[0].value).toBe('这是一个简单的内容');
    });
    it('应该执行完整处理流程', async () => {
        ...
        // 处理文档
        const result = await processor.process(complexDocument, '/test/complex-document.xml');
        // 验证结果
        expect(result).toBeDefined();
        expect(result.type).toBe(NodeType.DOCUMENT);
        // 验证文档结构
