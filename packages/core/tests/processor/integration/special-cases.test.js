...
describe('特殊场景测试', () => {
    ...
    it('应该处理特殊字符内容', async () => {
        ...
        // 验证特殊字符被正确处理
        const documentElement = result.children[0];
        const sectionElement = documentElement.children[0];
        const content = sectionElement.children[0];
    ...
    it('应该处理标签名大小写敏感性', async () => {
        ...
        const upperSection = documentElement.children[0];
        const lowerSection = documentElement.children[1];
        expect(upperSection.tagName).toBe('Section');
        expect(lowerSection.tagName).toBe('section');
        // 检查内容是否正确
        const upperContent = upperSection.children[0];
        const lowerContent = lowerSection.children[0];
        expect(upperContent.value).toContain('大写标签部分');
        expect(lowerContent.value).toContain('小写标签部分');
    });
    it('应该处理无根元素文档', async () => {
        // 创建多个根元素的文档
        const multiRootDocument = {
            type: NodeType.DOCUMENT,
            children: [
                {
                    type: NodeType.ELEMENT,
        ...
        // 检查内容是否正确
        const content1 = root1.children[0];
        const content2 = root2.children[0];
