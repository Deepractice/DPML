...
describe('复杂功能集成测试', () => {
    ...
    it('应该处理继承+引用组合场景', async () => {
        ...
        const inheritanceVisitor = new InheritanceVisitor(mockReferenceResolver);
        ...
        // 获取文档元素和基础组件元素
        const docElement = document.children[0];
        const baseComponent = docElement.children[0];
        const extendedComponent = docElement.children[1];
        // 手动将基础组件添加到ID映射表，以便继承访问者可以找到它
        context.idMap = context.idMap || new Map();
        context.idMap.set('baseComponent', baseComponent);
        // 验证基础组件
        ...
        expect(processedExtended.children[0].value).toBe('基础内容');
    });
    it('应该处理多级嵌套处理', async () => {
        // 创建一个简单的多级嵌套文档
        const document = {
        ...
        const docElement = result.children[0];
        // 验证各级嵌套结构
        const level1 = docElement.children[0];
        ...
        // 验证最深层内容
        const content = level3.children[0];
    ...
    it('应该模拟引用处理', async () => {
        ...
        // 验证引用节点被处理
        const docElement = document.children[0];
        const referenceNode = docElement.children[0];
        // 手动处理引用节点验证功能
        ...
        // 验证内容引用处理
        const section = docElement.children[1];
...
//# sourceMappingURL=complex-processing.test.js.map