...
import { ProcessingContext as ProcessingContextImpl } from '../../src/processor/processingContext';
describe('DefaultProcessor', () => {
    ...
    describe('访问者注册和排序机制', () => {
        it('应该能够注册访问者', () => {
            ...
            // 验证访问者已注册
            expect(processor.visitors).toContain(mockVisitor);
        });
        it('应该按优先级排序访问者', () => {
            // 创建多个优先级不同的访问者
        ...
        it('应该能注册协议处理器', () => {
            ...
            // 验证协议处理器已注册
            expect(processor.protocolHandlers).toContain(mockHandler);
        });
        it('应该能设置引用解析器', () => {
            // 创建模拟引用解析器
            const mockResolver = {
            ...
            // 验证引用解析器已设置
            expect(processor.referenceResolver).toBe(mockResolver);
        });
    });
    describe('处理流程骨架', () => {
        it('应该初始化处理上下文', async () => {
    ...
    describe('节点遍历逻辑', () => {
        it('应该递归遍历所有节点', async () => {
            ...
            // 验证访问者收到了正确的节点
            expect(trackingVisitor.visitDocument).toHaveBeenCalledWith(nestedDocument, expect.any(Object));
            expect(trackingVisitor.visitElement).toHaveBeenCalledWith(nestedDocument.children[0], expect.any(Object));
            expect(trackingVisitor.visitElement).toHaveBeenCalledWith(nestedDocument.children[0].children[0], expect.any(Object));
            expect(trackingVisitor.visitContent).toHaveBeenCalledWith(nestedDocument.children[0].children[0].children[0], expect.any(Object));
            expect(trackingVisitor.visitReference).toHaveBeenCalledWith(nestedDocument.children[0].children[1], expect.any(Object));
        });
        it('应该维护正确的父元素栈', async () => {
            ...
            // 存储父元素栈的状态
            let parentStackForChild = [];
            // 创建一个记录父元素栈的访问者
            const parentTrackingVisitor = {
                priority: 10,
                visitDocument: vi.fn().mockImplementation(doc => doc),
...
//# sourceMappingURL=defaultProcessor.test.js.map