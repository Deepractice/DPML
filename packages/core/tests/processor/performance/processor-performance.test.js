...
// 生成大型文档的工具函数
function generateLargeDocument(sectionsCount, elementsPerSection) {
    const document = {
    ...
    for (let i = 0; i < sectionsCount; i++) {
        const section = {
            ...
            children: [],
            position: {
                start: { line: i * elementsPerSection + 2, column: 1, offset: i * 1000 },
                end: { line: i * elementsPerSection + 2 + elementsPerSection, column: 1, offset: i * 1000 + 999 }
            }
        };
        // 添加内容元素
        for (let j = 0; j < elementsPerSection; j++) {
            ...
            if (j % 10 === 0) {
                section.children.push({
                    ...
                    children: [],
                    position: {
                        start: { line: i * elementsPerSection + j + 3, column: 1, offset: i * 1000 + j * 100 },
                        end: { line: i * elementsPerSection + j + 3, column: 50, offset: i * 1000 + j * 100 + 49 }
                    }
                });
            }
            else {
                    ...
                    children: [
                        {
                            ...
                            value: `这是章节 ${i} 中的段落 ${j}，包含一些文本内容。`.repeat(3), // 重复内容使其更大
                            position: {
                                start: { line: i * elementsPerSection + j + 3, column: 1, offset: i * 1000 + j * 100 },
                                end: { line: i * elementsPerSection + j + 3, column: 100, offset: i * 1000 + j * 100 + 99 }
                            }
                        }
                    ],
                    position: {
...
describe('处理器性能测试', () => {
    ...
    beforeEach(() => {
        ...
        processor.registerVisitor(new NamedReferenceVisitor({
            referenceResolver,
    ...
    it('应该正确使用引用缓存提升性能', async () => {
        ...
        // 设置mock元素作为ID引用
        const mockProcessingContext = new ProcessingContext(document, '/test/cache-test.xml');
        mockProcessingContext.idMap = new Map();
        mockProcessingContext.idMap.set('repeated-ref', createMockElement('mock', 'repeated-ref'));
        mockProcessingContext.resolvedReferences = new Map(); // 初始化resolvedReferences
        // 为了测试目的修改处理器的上下文
        processor.context = mockProcessingContext;
        ...
        // 使用mock替换handle方法
        IdProtocolHandler.prototype.handle = async function (reference) {
            handleCallCount++;
    ...
    it('应该测量关键路径的处理性能', async () => {
                            ...
                            children: [
                                {
                                    ...
                                    children: [
                                        {
                                            type: NodeType.CONTENT,
                                            value: '这是第一段落内容，包含一些**Markdown**格式和一个[[id:link1]]引用。',
                                            position: {
        ...
        const timings = {};
        // 预先设置上下文和引用
        const mockContext = new ProcessingContext(typicalDocument, '/test/performance-test.xml');
        mockContext.idMap = new Map();
        ...
        // 为测试目的修改处理器的上下文
        processor.context = mockContext;
        // 测量整体处理时间
        const startTime = performance.now();
        // 创建一个代理来记录处理时间
        const visitors = processor.visitors;
        const originalVisitElements = new Map();
        // 为每个访问者包装visitElement方法
        visitors.forEach(visitor => {
            if (visitor.visitElement) {
                originalVisitElements.set(visitor, visitor.visitElement);
                visitor.visitElement = async function (element, context) {
                    const start = performance.now();
                    const result = await originalVisitElements.get(visitor).call(this, element, context);
        ...
        try {
            ...
            if (timings['MarkdownContentVisitor']) {
                expect(timings['MarkdownContentVisitor']).toBeLessThan(timings['total'] * 0.5); // 应该不超过总时间的50%
            }
        }
        finally {
            // 恢复原始方法
            visitors.forEach(visitor => {
                if (originalVisitElements.has(visitor)) {
                    visitor.visitElement = originalVisitElements.get(visitor);
                }
            });
        }
    });
});
//# sourceMappingURL=processor-performance.test.js.map