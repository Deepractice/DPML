...
class CustomVisitor {
    constructor(priority) {
        this.name = 'CustomVisitor';
        this.priority = 100; // 默认优先级
        this.visitCalled = false;
        this.visitElementCalled = false;
        this.visitContentCalled = false;
        ...
        if (element.children && element.children.length > 0) {
            ...
            for (const child of element.children) {
                if (child.type === NodeType.CONTENT) {
                    newChildren.push(await this.visitContent(child, context));
                }
                else {
...
class AnotherVisitor {
    constructor(priority) {
        this.name = 'AnotherVisitor';
        this.priority = 50; // 较低的优先级
        this.visitCalled = false;
        if (priority !== undefined) {
            this.priority = priority;
        }
...
describe('访问者扩展能力测试', () => {
    ...
    it('应该能注册和执行自定义访问者', async () => {
        ...
        const resultElement = result.children[0];
        expect(resultElement.attributes['data-processed']).toBe('by-custom-visitor');
        // 验证内容处理
        const contentNode = resultElement.children[0];
    ...
    it('应该允许访问者修改上下文数据供后续访问者使用', async () => {
        // 创建一个向上下文添加数据的访问者
        class ContextSetterVisitor {
            constructor() {
                this.name = 'ContextSetterVisitor';
                this.priority = 150;
                this.executed = false;
            }
            async visitElement(element, context) {
        ...
        // 创建一个从上下文读取数据的访问者
        class ContextReaderVisitor {
            constructor() {
                this.name = 'ContextReaderVisitor';
                this.priority = 50;
...
//# sourceMappingURL=visitor-extension.test.js.map