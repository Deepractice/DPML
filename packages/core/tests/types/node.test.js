...
import { NodeType, isNode, isDocument, isElement, isContent, isReference } from '../../src/types/node';
describe('节点类型系统', () => {
    describe('节点类型枚举', () => {
        it('应该定义了所有必需的节点类型', () => {
    ...
    describe('基础节点类型', () => {
        ...
        it('应该可以区分不同类型的节点', () => {
            const node = { type: NodeType.ELEMENT, position: { start: { line: 1, column: 1, offset: 0 }, end: { line: 1, column: 10, offset: 9 } } };
            expect(isNode(node)).toBe(true);
    ...
    describe('Reference节点', () => {
        it('应该具有正确的结构', () => {
            ...
            expect(resolvedReference.resolved).toEqual({ content: 'resolved content' });
        });
    });
    describe('序列化与反序列化', () => {
        it('应该能够正确序列化和反序列化节点', () => {
            const doc = {
                type: NodeType.DOCUMENT,
                position: { start: { line: 1, column: 1, offset: 0 }, end: { line: 10, column: 1, offset: 100 } },
                children: [
                    {
                        type: NodeType.ELEMENT,
                        position: { start: { line: 2, column: 1, offset: 10 }, end: { line: 2, column: 20, offset: 30 } },
                        tagName: 'test-tag',
                        attributes: { id: 'test' },
                        children: [
                            {
                                type: NodeType.CONTENT,
                                position: { start: { line: 3, column: 1, offset: 40 }, end: { line: 3, column: 15, offset: 55 } },
                                value: 'This is content'
                            }
                        ]
                    }
            ...
            const element = deserialized.children[0];
            expect(element.tagName).toBe('test-tag');
