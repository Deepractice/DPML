...
import { NodeType, isElement, isContent } from '../../src/types/node';
describe('DpmlAdapter', () => {
    ...
    describe('基本功能', () => {
        it('应该成功处理简单的DPML内容', async () => {
            ...
            const contentNode = rootElement.children[0];
            expect(isContent(contentNode)).toBe(true);
            expect(contentNode.value).toBe('这是一个简单的提示');
        });
      ...
      <prompt>
        ...
        <role name="user">
          这是用户输入
            ...
            const contextElement = rootElement.children[0];
            expect(contextElement.tagName).toBe('context');
            const roleElement = rootElement.children[1];
    ...
    describe('Element处理', () => {
        it('应该正确保留元素的属性', async () => {
            ...
            const result = await dpmlAdapter.parse(dpml);
            const promptElement = result.ast.children[0];
        ...
        it('应该正确处理自闭合标签', async () => {
            ...
            const hrElement = promptElement.children[0];
            expect(hrElement.tagName).toBe('hr');
            expect(hrElement.children).toHaveLength(0);
            const brElement = promptElement.children[1];
    ...
    describe('Content处理', () => {
        it('应该正确处理纯文本内容', async () => {
            ...
            const result = await dpmlAdapter.parse(dpml);
            const promptElement = result.ast.children[0];
            const contentNode = promptElement.children[0];
      ...
      <prompt>
        ...
        这是更多的文本
            ...
            const emphasisElement = promptElement.children.find(child => child.type === NodeType.ELEMENT && child.tagName === 'emphasis');
            ...
            expect(emphasisElement.children[0].value).toBe('这是强调的文本');
        });
    });
});
//# sourceMappingURL=dpml-adapter.test.js.map