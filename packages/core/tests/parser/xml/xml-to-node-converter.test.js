...
describe('XML到DPML节点转换器', () => {
    describe('基础转换', () => {
        ...
        it('应该将XML子节点转换为Element节点', () => {
            ...
            const elementNode = result.children[0];
        ...
        it('应该将XML文本内容转换为Content节点', () => {
            ...
            const contentNode = elementNode.children[0];
    ...
    describe('嵌套结构', () => {
        it('应该正确处理深层嵌套结构', () => {
            ...
            const result = converter.convert(xmlNode);
            expect(result.children).toHaveLength(1);
            const sectionNode = result.children[0];
            expect(sectionNode.tagName).toBe('section');
            expect(sectionNode.children).toHaveLength(2);
            const headingNode = sectionNode.children[0];
            expect(headingNode.tagName).toBe('heading');
            expect(headingNode.children).toHaveLength(1);
            expect(headingNode.children[0].value).toBe('标题');
            const paragraphNode = sectionNode.children[1];
            expect(paragraphNode.tagName).toBe('paragraph');
            expect(paragraphNode.children).toHaveLength(1);
            expect(paragraphNode.children[0].value).toBe('段落内容');
        });
    });
});
//# sourceMappingURL=xml-to-node-converter.test.js.map