...
import { Element, NodeType, Document } from '../../../src/types/node';
import { TransformContext } from '../../../src/transformer/interfaces/transformContext';
import { TagProcessor, TagProcessorRegistry } from '../../../src/transformer/interfaces/tagProcessor';
...
describe('TagProcessorVisitor', () => {
  ...
  beforeEach(() => {
    ...
    // 创建访问者实例
    visitor = new TagProcessorVisitor(mockRegistry as unknown as TagProcessorRegistry);
  });
  ...
  it('应该处理处理器链和中间结果传递', async () => {
    ...
    // 模拟注册表返回处理器链
    mockRegistry.getProcessors.mockReturnValue([firstProcessor, secondProcessor]);
    mockRegistry.hasProcessors.mockReturnValue(true);
