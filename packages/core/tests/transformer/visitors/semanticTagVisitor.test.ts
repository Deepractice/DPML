...
import { SemanticTagVisitor } from '../../../src/transformer/visitors/semanticTagVisitor';
...
import { TagProcessorRegistry } from '../../../src/transformer/interfaces/tagProcessor';
...
describe('SemanticTagVisitor', () => {
  ...
  it('应该按优先级顺序应用多个处理器', async () => {
    ...
    mockRegistry.getProcessors.mockReturnValue([lowPriorityProcessor, highPriorityProcessor]);
    mockRegistry.hasProcessors.mockReturnValue(true);
  ...
  it('应该递归处理子元素', async () => {
    ...
    const childElement = {
      ...
      attributes: {},
      children: [],
      position: { start: { line: 2, column: 2, offset: 10 }, end: { line: 2, column: 10, offset: 18 } }
    };
    ...
    const parentElement = {
      ...
      position: { start: { line: 1, column: 1, offset: 0 }, end: { line: 3, column: 1, offset: 30 } }
    };
    
    const document: ProcessedDocument = {
      type: NodeType.DOCUMENT,
      children: [parentElement],
      position: { start: { line: 0, column: 0, offset: 0 }, end: { line: 4, column: 0, offset: 40 } }
    };
