...
describe('DefaultTransformer', () => {
    ...
    describe('访问者注册与排序', () => {
        ...
        it('应该能注册访问者', () => {
            ...
            // 检查内部访问者数组是否包含注册的访问者
            expect(transformer.visitors).toContain(visitor);
        });
        it('应该根据优先级排序访问者', () => {
            ...
            // 验证内部访问者数组的排序
            const visitors = transformer.visitors;
    ...
    describe('输出适配器', () => {
        ...
        it('应该能设置输出适配器', () => {
            ...
            // 检查内部适配器是否被设置
            expect(transformer.outputAdapter).toBe(adapter);
        });
        it('应该在转换后应用适配器', () => {
        ...
        it('应该处理包含子节点的文档', () => {
            ...
            const documentVisitor = {
                visitDocument: vi.fn().mockImplementation((doc, context) => {
                    ...
                    const childResults = doc.children.map((child) => transformer.transformNode(child, context));
        ...
        it('应该支持嵌套子节点处理', () => {
            ...
            const elementVisitor = {
                visitElement: vi.fn().mockImplementation((element, context) => {
                    const childResults = element.children.map((child) => transformer.transformNode(child, context));
                    return {
                        type: 'element',
        ...
        it('应该基于节点ID或内容哈希进行缓存', () => {
            ...
            const countingVisitor = {
                visitDocument: vi.fn().mockImplementation((doc) => {
                    visitCount.count++;
                    return { type: 'doc', id: doc.id, transformed: true };
                }),
                priority: 100
            };
            transformer.registerVisitor(countingVisitor);
...
//# sourceMappingURL=defaultTransformer.test.js.map