...
} from '../../../src/transformer/utils/variableConfig';
...
describe('VariableConfig', () => {
  ...
  describe('DefaultVariableResolver', () => {
    it('应该能解析基本变量引用', () => {
      ...
      expect(resolver.resolve('Hello, ${name}!', variables)).toBe('Hello, John!');
      expect(resolver.resolve('Age: ${age}', variables)).toBe('Age: 30');
      expect(resolver.resolve('Active: ${isActive}', variables)).toBe('Active: true');
    });
    ...
    it('应该能解析嵌套变量引用', () => {
      ...
      expect(resolver.resolve('User: ${user.name}', variables)).toBe('User: John');
      expect(resolver.resolve('Avatar: ${user.profile.avatar}', variables)).toBe('Avatar: john.jpg');
      expect(resolver.resolve('Theme: ${user.profile.settings.theme}', variables)).toBe('Theme: dark');
    });
    ...
    it('应该保留未定义变量的原始引用', () => {
      ...
      expect(resolver.resolve('Hello, ${unknown}!', variables)).toBe('Hello, ${unknown}!');
      expect(resolver.resolve('Theme: ${user.settings.theme}', variables)).toBe('Theme: ${user.settings.theme}');
    });
  ...
  describe('applyVariablesToContext', () => {
    ...
    it('应该支持控制深度合并行为', () => {
      ...
      // 浅合并
      const shallowContext = applyVariablesToContext(context, newVariables, false);
