...
describe('访问者优先级机制', () => {
  ...
  it('应该按照优先级从高到低执行访问者', () => {
    ...
    const lowPriorityVisitor: TransformerVisitor = {
      priority: 10,
      visitDocument: (document: ProcessedDocument, context: TransformContext) => {
  ...
  it('应该为没有指定优先级的访问者使用默认优先级', () => {
    ...
    const visitorWithPriority: TransformerVisitor = {
      priority: 100,
      visitDocument: (document: ProcessedDocument, context: TransformContext) => {
        callOrder.push('有优先级');
        return null;
    ...
    const visitorWithoutPriority1: TransformerVisitor = {
      visitDocument: (document: ProcessedDocument, context: TransformContext) => {
    ...
    const visitorWithoutPriority2: TransformerVisitor = {
      visitDocument: (document: ProcessedDocument, context: TransformContext) => {
  ...
  it('应该支持对不同类型节点设置不同的访问者优先级', () => {
    ...
    const nodeTypeOrder: string[] = [];
    
    const documentVisitor: TransformerVisitor = {
      priority: 50,
      visitDocument: (doc: ProcessedDocument, context: TransformContext) => {
    ...
    const elementVisitor: TransformerVisitor = {
      ...
      visitElement: (element: any, context: TransformContext) => {
