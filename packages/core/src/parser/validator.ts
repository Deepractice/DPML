...
import { Node, NodeType, Element, Document, isElement } from '../types/node';
...
export class Validator {
   ...
   * @returns 验证错误对象
   */
  private createError(code: string, message: string, position?: SourcePosition): ValidationError {
    return {
      code,
      message,
      position
   ...
   * @returns 验证警告对象
   */
  private createWarning(code: string, message: string, position?: SourcePosition): ValidationWarning {
    return {
      code,
      message,
      position
