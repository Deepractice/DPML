...
describe('边界条件测试', () => {
  ...
  it('应该处理大型文档', async () => {
    ...
    for (let i = 0; i < NESTING_DEPTH; i++) {
        ...
        const siblingElement: Element = {
          ...
          children: [
            {
              ...
              position: {
                start: {
                  line: 100 + i * 10 + j,
                  column: 1,
                  offset: 1000 + i * 1000 + j * 100,
                },
                end: {
                  line: 100 + i * 10 + j + 1,
                  column: 1,
                  offset: 1000 + i * 1000 + j * 100 + 50,
                },
              },
            } as Content,
          ],
          position: {
            start: {
              line: 100 + i * 10 + j - 1,
              column: 1,
              offset: 1000 + i * 1000 + j * 100 - 10,
            },
            end: {
              line: 100 + i * 10 + j + 2,
              column: 1,
              offset: 1000 + i * 1000 + j * 100 + 60,
            },
          },
        };
      ...
      currentParent = levelElement as Element;
    }
    ...
    // 验证文档结构保持不变（深度和宽度）
    let resultElement = result.children[0] as Element;
    ...
    while (resultElement.children.length > 0 && depth < NESTING_DEPTH) {
      // 检查每层的元素数量
      expect(resultElement.children.length).toBeGreaterThanOrEqual(
        ELEMENTS_PER_LEVEL
      );

      // 检查下一层的第一个元素
      const nextElement = resultElement.children[0] as Element;
    ...
    if (depth === NESTING_DEPTH) {
      ...
      const content = resultElement.children[0] as Content;
    ...
    if (context) {
      ...
      // 验证所有ID都被收集
      expect(idMap.size).toBeGreaterThanOrEqual(
        NESTING_DEPTH + NESTING_DEPTH * (ELEMENTS_PER_LEVEL - 1)
      );
  ...
  it('应该处理异常情况', async () => {
    ...
    strictProcessor.registerVisitor(
      new AttributeValidationVisitor({
        tagRegistry,
    ...
    // 创建一个有重复ID的文档
    const duplicateIdDocument: Document = {
      type: NodeType.DOCUMENT,
      children: [
        {
          type: NodeType.ELEMENT,
    ...
    const result = await processor.process(
      duplicateIdDocument,
      '/test/duplicate-id.xml'
    );

    expect(result).toBeDefined();

    // 2. 测试引用不存在的ID
    const nonExistentIdDocument: Document = {
      type: NodeType.DOCUMENT,
      children: [
        {
          type: NodeType.ELEMENT,
