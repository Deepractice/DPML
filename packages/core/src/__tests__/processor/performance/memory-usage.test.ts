...
function generateHugeDocument(
  ...
  // 创建一个大字符串用于内容填充
  const largeContent =
    '这是一段用于填充内容的文本，用于测试内存使用情况。'.repeat(contentSize);
        ...
        section.children.push({
          ...
          position: {
            start: {
              ...
              column: 1,
              offset: i * 10000 + j * 100,
            },
            end: {
              line: i * elementsPerSection + j + 3,
              column: 50,
              offset: i * 10000 + j * 100 + 49,
            },
          },
        } as Element);
      } else {
...
describe('处理器内存使用测试', () => {
  ...
  it('应该限制内存增长在合理范围内', async () => {
    ...
    console.log(
      `中等文档处理后内存: ${(mediumMemory.heapUsed / 1024 / 1024).toFixed(2)} MB`
    );
    console.log(
      `内存增长: ${((mediumMemory.heapUsed - initialMemory.heapUsed) / 1024 / 1024).toFixed(2)} MB`
    );
    ...
    console.log(
      `GC后内存使用: ${(afterGcMemory.heapUsed / 1024 / 1024).toFixed(2)} MB`
    );
    ...
    console.log(
      `大型文档处理后内存: ${(largeMemory.heapUsed / 1024 / 1024).toFixed(2)} MB`
    );
    console.log(
      `内存增长: ${((largeMemory.heapUsed - afterGcMemory.heapUsed) / 1024 / 1024).toFixed(2)} MB`
    );
    ...
    // 临时禁用验证，直到 GC 问题解决
    console.log(
      `内存回收比例: ${((memoryReclaimed / memoryGrowth) * 100).toFixed(2)}%`
    );
  });
  ...
  it('应该优化深层次嵌套结构的内存使用', async () => {
    ...
    console.log(
      `处理前内存使用: ${(beforeProcessMemory.heapUsed / 1024 / 1024).toFixed(2)} MB`
    );
    ...
    console.log(
      `处理后内存使用: ${(afterProcessMemory.heapUsed / 1024 / 1024).toFixed(2)} MB`
    );
    console.log(
      `内存增长: ${((afterProcessMemory.heapUsed - beforeProcessMemory.heapUsed) / 1024 / 1024).toFixed(2)} MB`
    );
  ...
  it('应该高效处理引用解析的内存使用', async () => {
    ...
    // 处理文档
    processor.setReferenceResolver(
      new DefaultReferenceResolver({ useCache: true })
    );
