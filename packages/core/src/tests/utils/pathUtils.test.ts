...
describe('路径处理工具测试', () => {
  ...
  describe('getFileName', () => {
    ...
    it('应该获取URL的文件名', () => {
      expect(getFileName('https://example.com/path/to/file.txt')).toBe(
        'file.txt'
      );
      expect(
        getFileName('https://example.com/path/to/file.txt?query=value')
      ).toBe('file.txt');
    });
  });
  ...
  describe('getDirName', () => {
    ...
    it('应该获取URL的目录名', () => {
      expect(getDirName('https://example.com/path/to/file.txt')).toBe(
