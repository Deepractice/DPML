...
} from '../../../transformer/utils/errorFormatter';
import { DEFAULT_LOOSE_MODE } from '../../../transformer/utils/modeConfig';
...
describe('错误消息格式化和处理', () => {
  ...
  it('应该根据详细程度生成不同格式的日志消息', () => {
    ...
    const minimalConfig = {
      ...DEFAULT_LOOSE_MODE,
      errorVerbosity: 'minimal' as const,
    };
    ...
    const detailedConfig = {
      ...DEFAULT_LOOSE_MODE,
      errorVerbosity: 'detailed' as const,
    };
  ...
  it('应该在异步转换中提供更丰富的错误信息', async () => {
    ...
    // 确认错误消息包含异步错误
    const errorMessages = consoleErrorSpy.mock.calls.map(
      (call: any[]) => call[0]
    );
    const asyncErrors = errorMessages.filter(
      (msg: string) => typeof msg === 'string' && msg.includes('异步错误消息')
    );
