...
describe('父结果传递机制', () => {
  ...
  it('应该支持路径和父结果的组合使用', () => {
    ...
    const pathRecordVisitor = {
      ...
      visitElement: vi.fn((element: any, context: TransformContext) => {
        pathsWithParentResults.push({
          ...
          parentCount: context.parentResults.length,
          lastParentType:
            context.parentResults.length > 0
              ? context.parentResults[context.parentResults.length - 1].type
              : null,
        });
    ...
    // section层级
    expect(pathsWithParentResults[1].path).toEqual([
      'document',
      'children',
      'element[section]',
    ]);
    ...
    // paragraph层级
    expect(pathsWithParentResults[2].path).toEqual([
      'document',
      'children',
      'element[section]',
      'children',
    ...
    // content层级
    expect(pathsWithParentResults[3].path).toEqual([
      'document',
      'children',
      'element[section]',
      'children',
  ...
  it('应该在嵌套处理中正确传递父结果', () => {
    ...
    expect(visitLog[3].parentResults).toEqual([
      'document',
      'section',
      'paragraph',
    ]);
  ...
  it('应该在异步转换中正确传递父结果', async () => {
    ...
    const asyncTracingVisitor = {
      ...
      visitDocumentAsync: vi.fn(
        async (document: any, context: TransformContext) => {
          asyncVisitLog.push({
      ...
      visitElementAsync: vi.fn(
        async (element: any, context: TransformContext) => {
          asyncVisitLog.push({
      ...
      visitContentAsync: vi.fn(
        async (content: any, context: TransformContext) => {
          asyncVisitLog.push({
    ...
    expect(asyncVisitLog[3].parentResults).toEqual([
      'document',
      'section',
      'paragraph',
    ]);
