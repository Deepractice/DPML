...
import type { TagProcessorRegistry } from '../../transformer/interfaces/tagProcessorRegistry';
...
describe('TagProcessorRegistry', () => {
  ...
  it('应该能注册和获取处理器', () => {
    ...
    expect(roleProcessor?.process(roleElement, mockContext)).toEqual({
      type: 'role-processed',
    });

    expect(promptProcessor?.canProcess(promptElement)).toBe(true);
    expect(promptProcessor?.process(promptElement, mockContext)).toEqual({
      type: 'prompt-processed',
    });
  });
