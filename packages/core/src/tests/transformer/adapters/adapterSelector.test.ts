...
import type { AdapterSelector } from '../../../transformer/interfaces/adapterSelector';
...
describe('AdapterSelector', () => {
  ...
  it('应该支持基于内容类型的格式推断', () => {
    ...
    const result = {
      type: 'document',
      content: '<root><item>测试</item></root>',
    };
  ...
  it('应该支持自定义格式解析逻辑', () => {
    ...
    const adapter2 = customSelector.selectAdapter(null, context, {
      type: 'markdown',
      content: '# 标题',
    });
