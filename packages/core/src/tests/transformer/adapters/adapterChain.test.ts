...
import { DefaultAdapterChain } from '../../../transformer/adapters/defaultAdapterChain';
...
import type { AdapterChain } from '../../../transformer/interfaces/adapterChain';
...
describe('AdapterChain', () => {
  ...
  // 创建自定义测试适配器
  class CounterAdapter implements OutputAdapter {
    count = 0;
  ...
  class ThrowingAdapter implements OutputAdapter {
    adapt(result: any, context: TransformContext): any {
  ...
  it('当stopOnEmpty为true时，应该在遇到null结果时停止', () => {
    ...
    expect((adapter3 as CounterAdapter).count).toBe(0); // 第三个适配器没有执行
  });
  ...
  it('当stopOnError为false时，应该继续执行后续适配器', () => {
    ...
    // 验证
    expect((adapter3 as CounterAdapter).count).toBe(1); // 第三个适配器应该执行
  });
