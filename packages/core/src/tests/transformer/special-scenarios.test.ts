...
import { SpecialScenariosVisitor } from '../../transformer/visitors/specialScenariosVisitor';
...
describe('特殊场景测试', () => {
  ...
  describe('混合格式内容处理', () => {
    it('应该能处理包含Markdown和HTML混合内容', () => {
      ...
      expect(result.children[0].children[0].value).toContain(
        '&lt;span style=&quot;color:red&quot;&gt;HTML内容&lt;/span&gt;'
      );
    });
  });
    ...
    it('应该能处理不规则和深度嵌套的标签结构', () => {
      ...
      const level2bElement: Element = {
        ...
        position: {
          ...
          end: { line: 7, column: 10, offset: 80 },
        },
      };
      ...
      expect(result.children[0].children[0].children[0].children[0].value).toBe(
        '深度嵌套内容'
      );
            ...
            children: [
              {
                type: NodeType.CONTENT,
                value:
                  '${greeting}，${user.name}！你的得分是${user.scores.total}分。',
                position: {
                  start: { line: 1, column: 10, offset: 9 },
                  end: { line: 1, column: 50, offset: 49 },
                },
