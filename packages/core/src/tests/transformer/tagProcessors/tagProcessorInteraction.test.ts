...
} from '../../../transformer/interfaces/tagProcessor';
...
describe('标签处理器交互', () => {
  ...
  it('应该按优先级顺序执行处理器链', async () => {
    ...
    expect(result.meta?.processedBy).toEqual([
      'processor2',
      'processor3',
      'processor1',
    ]);
  ...
  it('应该支持处理器之间的数据传递', async () => {
    ...
    const dataTransformProcessor: TagProcessor = {
      ...
      canProcess(element: Element): boolean {
        // 只处理已经被初始化的元素
        return (
          element.tagName === 'data-tag' && element.meta?.initialized === true
        );
      },
      process(element: Element, context: TransformContext): Element {
        // 获取第一个处理器设置的值并处理
    ...
    const dataFinalizeProcessor: TagProcessor = {
      ...
      canProcess(element: Element): boolean {
        // 只处理已经被转换的元素
        return (
          element.tagName === 'data-tag' && element.meta?.transformed === true
        );
      },
      process(element: Element, context: TransformContext): Element {
        // 获取前一个处理器设置的值并最终处理
    ...
    const childProcessor: TagProcessor = {
      ...
      process(element: Element, context: TransformContext): Element {
        ...
        const parentMeta =
          ...
          parentResults[parentResults.length - 1]?.meta;
