...
describe('处理器链执行机制', () => {
    ...
    addConditionalProcessor(
      ...
      const processor: TagProcessor = {
        ...
        canProcess: (element: Element) =>
          element.tagName === tagName && condition(element),
        process: (element: Element, context: TransformContext) => {
          this.executionOrder.push(name);
  ...
  it('应该正确处理异步处理器链', async () => {
    ...
    expect(chainBuilder.getExecutionOrder()).toEqual([
      'async1',
      'async2',
      'async3',
    ]);
  ...
  it('应该正确处理处理器链中的错误', async () => {
    ...
    chainBuilder
      ...
      .addErrorProcessor(
        ...
        'error',
        50,
        'Intentional error in processor chain'
      )
  ...
  it('应该正确处理通配符处理器', async () => {
    ...
    expect(
      (otherResult.meta?.processedBy as string[]).every(p => p === 'wildcard')
    ).toBe(true);
    expect(chainBuilder.getExecutionOrder().every(p => p === 'wildcard')).toBe(
      true
    );
  });
});
