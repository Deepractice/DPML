...
import { NodeType, Element, Content, Document, Reference, Node } from '../../../types/node';
...
import { NodeVisitor } from '../../../processor/interfaces/nodeVisitor';
...
// 生成大型文档的工具函数
function generateLargeDocument(sectionsCount: number, elementsPerSection: number): Document {
  ...
  it('应该测量关键路径的处理性能', async () => {
    ...
    visitors.forEach(visitor => {
      if (visitor.visitElement) {
        ...
        visitor.visitElement = async function(element: Element, context: ProcessingContext): Promise<Element> {
          const start = performance.now();
          const result = await (originalVisitElements.get(visitor) as Function).call(this, element, context);
