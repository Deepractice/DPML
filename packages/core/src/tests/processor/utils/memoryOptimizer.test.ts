...
import { MemoryOptimizer } from '../../../processor/utils/memoryOptimizer';
import { NodeType } from '../../../types/node';

import type { Document, Element, Node } from '../../../types/node';
...
describe('内存优化器', () => {
  ...
  it('应该支持禁用迭代遍历', () => {
    ...
    // 验证递归方法被调用
    expect((nonIterativeOptimizer as any).traverseNode).toHaveBeenCalledWith(
      document,
      mockVisitor
    );
  });
