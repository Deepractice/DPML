...
import { NodeType, Document, Element, SourcePosition } from '../../../types/node';
...
describe('IdValidationVisitor', () => {
  let visitor: IdValidationVisitor;
  ...
  it('应该初始化idMap', async () => {
    ...
    // 验证idMap被初始化
    expect((context as any).idMap).toBeDefined();
    expect((context as any).idMap instanceof Map).toBe(true);
    expect((context as any).idMap?.size).toBe(0);
  });
  ...
  it('应该收集元素ID', async () => {
    ...
    // 验证ID被收集
    expect((context as any).idMap?.size).toBe(2);
    expect((context as any).idMap?.has('test1')).toBe(true);
    expect((context as any).idMap?.has('test2')).toBe(true);
    expect((context as any).idMap?.get('test1')).toBe(element1);
    expect((context as any).idMap?.get('test2')).toBe(element2);
  });
  ...
  it('在非严格模式下应该只发出重复ID警告', async () => {
    ...
    try {
      ...
      // 验证两个元素的ID都被收集
      expect((context as any).idMap?.size).toBe(1);
      expect((context as any).idMap?.has('duplicate')).toBe(true);
    } finally {
      // 恢复console.warn
  ...
  it('应该正确处理嵌套元素的ID', async () => {
    ...
    // 验证ID被收集
    expect((context as any).idMap?.size).toBe(2);
    expect((context as any).idMap?.has('parent-id')).toBe(true);
    expect((context as any).idMap?.has('child-id')).toBe(true);
  });
});
