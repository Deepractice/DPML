...
describe('Parser与Processor集成测试', () => {
  ...
  it('应该能正确处理Parser解析的文档', async () => {
    ...
    // 用处理器处理解析后的文档
    const processedDocument = await processor.process(
      document,
      'test-dpml.xml'
    );
    ...
    const countSections = node => {
      if (
        ...
        if (
          node.attributes.id === 'test-section' ||
          node.attributes.id === 'included-section'
        ) {
          sectionCount++;
        }
      }
  ...
  it('应该在处理过程中保留Parser解析的结构信息', async () => {
    ...
    // 验证section的子元素结构
    if (section) {
      const hasTitle = section.children.some(
        child =>
      ...
      const hasContent = section.children.some(
        child =>
          child.type === NodeType.ELEMENT &&
          child.attributes &&
  ...
  it('应该能处理任意根标签的XML文档', async () => {
    ...
    // 验证子元素也被正确处理
    const sectionElement = processedRoot.children.find(
      child =>
        child.type === NodeType.ELEMENT &&
