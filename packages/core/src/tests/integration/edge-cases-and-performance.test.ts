...
import { Document, Element, NodeType, Node } from '@core/types/node';
import { TextNode } from '@core/types/contentNode';
import { ProcessedDocument } from '@core/processor/interfaces/processor';
...
describe('边界情况和性能测试', () => {
  ...
  const adapterFactory: OutputAdapterFactory = new DefaultOutputAdapterFactory();
  ...
  afterEach(() => {
    ...
    if (memoryBefore && memoryAfter) {
      console.log('内存使用变化:');
      console.log(`- 堆总大小: ${(memoryAfter.heapTotal - memoryBefore.heapTotal) / 1024 / 1024} MB`);
      console.log(`- 堆已用: ${(memoryAfter.heapUsed - memoryBefore.heapUsed) / 1024 / 1024} MB`);
      console.log(`- RSS: ${(memoryAfter.rss - memoryBefore.rss) / 1024 / 1024} MB`);
    }
  ...
  // 辅助函数：创建大型测试文档
  function createLargeDocument(sections: number, elementsPerSection: number): Document {
  ...
  it('应该能高效处理大型文档 (性能测试)', async () => {
    ...
    const processedDoc = await processor.process(largeDocument, 'large-doc.dpml');
    ...
    console.log(`内存使用增加: ${((finalMemoryUsage.heapUsed - initialMemoryUsage.heapUsed) / (1024 * 1024)).toFixed(2)}MB`);
  ...
  it('应该在多次转换后保持稳定的内存使用 (内存监控)', async () => {
    ...
    // 初始内存使用
    const initialMemoryUsage = process.memoryUsage();
    console.log(`初始堆内存使用: ${(initialMemoryUsage.heapUsed / (1024 * 1024)).toFixed(2)}MB`);
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (i === 4 || i === 9) {
        ...
        console.log(`第 ${i+1} 次转换后堆内存使用: ${(currentMemoryUsage.heapUsed / (1024 * 1024)).toFixed(2)}MB`);
      }
    }
    ...
    console.log(`最终堆内存使用: ${(finalMemoryUsage.heapUsed / (1024 * 1024)).toFixed(2)}MB`);
  ...
  it('应该支持并发转换请求 (并发测试)', async () => {
    ...
    const results = await Promise.all(processedDocs.map(doc => 
