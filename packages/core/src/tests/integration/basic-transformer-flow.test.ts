...
describe('转换器基础流程集成测试', () => {
  ...
  it('应该能解析并转换简单文档为JSON', async () => {
    ...
    console.log('处理后的文档内容:', JSON.stringify(processedDoc, null, 2).substring(0, 500));
    ...
    console.log('JSON输出键:', jsonOutput ? Object.keys(jsonOutput).join(', ') : '无');
  ...
  it('应该支持处理嵌套复杂元素', async () => {
    ...
    expect(jsonOutput.document.section.container.list.item).toBeInstanceOf(Array);
    expect(jsonOutput.document.section.container.list.item.length).toBe(3);
    expect(jsonOutput.document.section.container.list.item[2].paragraph).toBe('嵌套段落');
    expect(jsonOutput.document.section.container.list.item[2].code).toBe('console.log(\'Hello\');');
  });
