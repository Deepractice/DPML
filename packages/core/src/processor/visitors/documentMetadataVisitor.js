...
(function (DocumentMode) {
     ...
     * 严格模式 - 所有验证错误都会导致处理失败
     */
    DocumentMode["STRICT"] = "strict";
    /**
     * 宽松模式 - 非致命错误会被记录为警告，不会中断处理
     */
    DocumentMode["LOOSE"] = "loose";
})(DocumentMode || (DocumentMode = {}));
/**
 * 文档元数据访问者
 * 收集文档的元数据信息
 */
export class DocumentMetadataVisitor {
    /**
     * 构造函数
     * @param options 选项
     */
    ...
    async visitDocument(document, context) {
        // 查找根元素
        const rootElement = document.children.find(child => child.type === 'element');
        if (!rootElement) {
            // 没有根元素，使用默认元数据
...
//# sourceMappingURL=documentMetadataVisitor.js.map