...
export class InheritanceVisitor implements NodeVisitor {
   ...
   * @returns 解析后的基础元素
   */
  private async resolveBaseElement(extendsValue: string, context: ProcessingContext): Promise<Element> {
    // ID引用格式: id:elementId
   ...
   * @returns 解析后的元素
   */
  private async resolveExternalReference(reference: string, context: ProcessingContext): Promise<Element> {
   ...
   * @returns [URL, ID]
   */
  private parseExternalReference(reference: string): [string, string | undefined] {
