...
export class MarkdownContentVisitor implements NodeVisitor {
   ...
   * 需要完全移除的危险标签列表
   */
  private readonly dangerousTags = ['script', 'style', 'iframe', 'object', 'embed'];
   ...
   * @returns 处理后的内容节点
   */
  async visitContent(content: Content, context: ProcessingContext): Promise<ContentWithMarkdown> {
    // 如果内容为空，直接返回
