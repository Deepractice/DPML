 ...
 */
export class InheritanceVisitor {
    /**
     * 创建继承访问者
    ...
    async visitElement(element, context) {
        ...
        context.variables.inheritanceChain = context.variables.inheritanceChain || [];
     ...
     * @returns 解析后的基础元素
     */
    async resolveBaseElement(extendsValue, context) {
        // ID引用格式: id:elementId
        ...
        if (extendsValue.startsWith('file:') || extendsValue.startsWith('http:') || extendsValue.startsWith('https:')) {
     ...
     * @returns 解析后的元素
     */
    async resolveExternalReference(reference, context) {
        ...
        const refObj = {
            ...
            protocol: url.startsWith('http:') || url.startsWith('https:') ? 'http' : 'file',
            path: url,
            position: {
     ...
     * @returns [URL, ID]
     */
    parseExternalReference(reference) {
     ...
     * @returns 合并后的元素
     */
    mergeElements(baseElement, childElement) {
        // 创建结果元素，基于子元素
        const result = {
            ...childElement,
...
//# sourceMappingURL=inheritanceVisitor.js.map