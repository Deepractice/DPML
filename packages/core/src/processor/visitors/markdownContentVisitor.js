 ...
 */
export class MarkdownContentVisitor {
    /**
     * 构造函数
    ...
    constructor(options = {}) {
         ...
         * 需要完全移除的危险标签列表
         */
        this.dangerousTags = ['script', 'style', 'iframe', 'object', 'embed'];
        this.options = {
            sanitize: false,
            breaks: false,
            gfm: true,
     ...
     */
    visit(node) {
        if (node.type === 'Content' && this.hasMarkdown(node)) {
            this.processMarkdown(node);
        }
    }
    /**
     * 处理内容节点
    ...
    async visitContent(content, context) {
        ...
        try {
            ...
            if (this.options.sanitize) {
                ...
                processedContent = processedContent.replace(htmlTagPattern, (match, tag, innerContent) => {
                    // 处理标签内的Markdown内容
                    const processedInner = marked.parse(innerContent);
                    const innerHtml = typeof processedInner === 'string'
                        ? processedInner
                        : processedInner.toString();
                    const cleanedInner = innerHtml.trim().replace(/^<p>([\s\S]*)<\/p>$/, '$1');
    ...
    hasMarkdown(node) {
        ...
        return markdownPatterns.some(pattern => pattern.test(node.value));
    }
    /**
     * 处理Markdown内容
     * @param node Content节点
...
//# sourceMappingURL=markdownContentVisitor.js.map