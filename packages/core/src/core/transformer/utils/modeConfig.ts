...
export function getModeConfig(options?: TransformOptions): ModeConfigOptions {
  ...
  // 根据mode选择基础配置
  const baseConfig =
    options.mode === 'strict' ? DEFAULT_STRICT_MODE : DEFAULT_LOOSE_MODE;
...
export function handleModeError(
  ...
  const isThresholdExceeded =
    config.errorThreshold > 0 && errorCount > config.errorThreshold;

  // 如果超过阈值且配置为中止转换，返回false表示不继续
  if (
    isThresholdExceeded &&
    config.thresholdExceededAction === 'abort-transform'
  ) {
    return false;
  }
