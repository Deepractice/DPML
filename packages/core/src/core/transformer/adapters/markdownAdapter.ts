 ...
 * 将结果转换为Markdown字符串
 */
export class MarkdownAdapter implements OutputAdapter {
  /**
   * 适配器选项
   * @private
   */
   ...
   * @private
   */
  private elementToMarkdown(element: any): string {
   ...
   * @private
   */
  private generateFrontmatter(meta: Record<string, any>): string {
    let frontmatter = '---\n';
   ...
   * @private
   */
  private headingToMarkdown(heading: any): string {
   ...
   * @private
   */
  private paragraphToMarkdown(paragraph: any): string {
    // 递归处理子节点
    let content = '';
   ...
   * @private
   */
  private listToMarkdown(list: any): string {
    if (
      !list.children ||
    ...
    // 处理列表项
    list.children.forEach((item: any, index: number) => {
      if (isOrdered) {
        // 有序列表使用数字+点
        markdown += `${index + 1}. `;
      } else {
   ...
   * @private
   */
  private listItemToMarkdown(item: any): string {
    // 递归处理子节点
    let content = '';
  ...
  private tableToMarkdown(table: any): string {
    ...
    for (const child of table.children) {
      if (
        child.name === 'tableHead' &&
        child.children &&
        Array.isArray(child.children)
      ) {
        headerRow = child.children[0]; // 假设表头只有一行
      } else if (
        child.name === 'tableBody' &&
        child.children &&
        Array.isArray(child.children)
      ) {
   ...
   * @private
   */
  private tableRowToMarkdown(row: any): string {
   ...
   * @private
   */
  private tableCellToMarkdown(cell: any): string {
    // 由tableToMarkdown处理，这里返回空字符串
    return '';
  }
   ...
   * @private
   */
  private codeBlockToMarkdown(codeBlock: any): string {
   ...
   * @private
   */
  private inlineCodeToMarkdown(inlineCode: any): string {
    // 获取代码内容
    let code = '';
   ...
   * @private
   */
  private strongToMarkdown(strong: any): string {
    // 获取内容
    let content = '';
   ...
   * @private
   */
  private emphasisToMarkdown(emphasis: any): string {
    // 获取内容
    let content = '';
   ...
   * @private
   */
  private linkToMarkdown(link: any): string {
    // 获取链接文本
    let text = '';
   ...
   * @private
   */
  private imageToMarkdown(image: any): string {
   ...
   * @private
   */
  private blockquoteToMarkdown(blockquote: any): string {
    // 获取内容
    let content = '';
