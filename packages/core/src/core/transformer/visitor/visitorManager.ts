...
export class VisitorManager {
  ...
  getVisitorsByMethod(methodName: string): TransformerVisitor[] {
    ...
    return this.visitors.filter(visitor => {
      // 检查访问者是否被禁用
      const isDisabled = visitor.name
        ? this.disabledVisitors.has(visitor.name)
        : false;
  ...
  disableVisitor(visitorName: string, reason: string = '未指定原因'): boolean {
    ...
    if (!this.disabledVisitors.has(visitorName)) {
      ...
      console.warn(
        `访问者 ${visitorName} 已禁用: ${reason}${errorCount > 0 ? `（错误次数：${errorCount}/${this.errorThreshold}）` : ''}`
      );
      ...
      if (this.autoRecoveryTime > 0) {
        ...
        console.warn(
          `[自动恢复] 访问者 ${visitorName} 将在 ${recoveryTimeInSeconds} 秒后自动恢复`
        );
      }
