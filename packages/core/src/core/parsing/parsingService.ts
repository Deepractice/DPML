 ...
 * @returns 解析后的DPML文档
 */
export function parse<T = DPMLDocument>(
  content: string,
  ...
  try {
    ...
    // 创建适配器
    const adapter = parserFactory.createDPMLAdapter(effectiveOptions);
 ...
 */
export async function parseAsync<T = DPMLDocument>(
  content: string,
  options: ParseOptions = {}
): Promise<T | ParseResult<T>> {
  try {
    // 在测试环境中使用原始选项，否则应用默认选项合并
    let effectiveOptions = isTestEnvironment ? options : mergeOptions(options);
...
function applyMemoryOptimization(
  ...
  if (shouldEnableOptimization === undefined) {
    const threshold =
      memoryOpt.largeFileThreshold ||
      DEFAULT_OPTIONS.memoryOptimization!.largeFileThreshold!;
 ...
 */
function handleParsingErrors<T>(
  error: unknown,
  options?: ParseOptions
): ParseResult<T> | never {
  // 将错误转换为ParseError类型
...
function convertToParseError(error: unknown, fileName?: string): ParseError {
  ...
  if (error instanceof Error) {
    ...
    return new ParseError(
      ...
      position ||
        (fileName
          ? { startLine: 0, startColumn: 0, endLine: 0, endColumn: 0, fileName }
          : undefined),
      undefined,
      error
    );
