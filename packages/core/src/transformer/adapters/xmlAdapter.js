...
export class XMLAdapter {
     ...
     * @private
     */
    convertToXml(value, nodeName, level) {
        ...
        if (Array.isArray(value)) {
            ...
            for (const item of value) {
                xml += this.convertToXml(item, this.options.itemName || 'item', level + 1) + newline;
            }
            xml += `${indent}</${nodeName}>`;
            return xml;
        }
        ...
        for (const key in value) {
            if (Object.prototype.hasOwnProperty.call(value, key)) {
                const val = value[key];
                // 特殊处理属性集合
                if (key === 'attributes' && typeof val === 'object' && !Array.isArray(val)) {
                    for (const attrKey in val) {
                        if (Object.prototype.hasOwnProperty.call(val, attrKey)) {
                            attributes.push(`${attrKey}="${this.escapeXml(String(val[attrKey]))}"`);
                        }
                    }
                    continue;
                }
                ...
                if (key === 'children' && Array.isArray(val)) {
                    for (const child of val) {
                        ...
                        else if (child && typeof child === 'object' && child.type === 'element') {
                            childName = 'element';
                        }
        ...
        if (attributes.length > 0) {
            ...
            if (nodeName === 'section' && value.attributes && value.attributes.id === 'section1') {
