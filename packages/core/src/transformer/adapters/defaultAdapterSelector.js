 ...
 * 基于格式、上下文和结果选择合适的适配器
 */
export class DefaultAdapterSelector {
    /**
     * 构造函数
     *
     * @param factory 适配器工厂
     ...
     */
    selectAdapter(format, context, result) {
        // 1. 优先使用显式传入的格式
        if (format) {
    ...
    inferFormatFromResult(result) {
        ...
        if (typeof result === 'object' && result !== null) {
            // 如果有type属性，根据type判断
            if (result.type) {
                if (result.type === 'document' && result.content && typeof result.content === 'string') {
                    // 递归检查content字段
...
//# sourceMappingURL=defaultAdapterSelector.js.map