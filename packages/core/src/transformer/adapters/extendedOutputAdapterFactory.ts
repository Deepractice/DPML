...
import { AdapterChain } from '../interfaces/adapterChain';
import { DefaultAdapterChain } from './defaultAdapterChain';

/**
 * 扩展的适配器工厂配置选项
 */
export interface ExtendedOutputAdapterFactoryOptions extends OutputAdapterFactoryOptions {
  /**
   * 格式别名映射
   * 
   ...
   */
  formatMapper?: (format: string, factory: OutputAdapterFactory) => OutputAdapter | null;
}
...
export class ExtendedOutputAdapterFactory implements OutputAdapterFactory {
   ...
   * @private
   */
  private formatMapper: ((format: string, factory: OutputAdapterFactory) => OutputAdapter | null) | null;
  ...
  supportsFormat(format: string): boolean {
    ...
    // 检查映射器支持（如果有映射器）
    if (this.formatMapper && this.formatMapper(normalizedFormat, this) !== null) {
      return true;
    }
   ...
   * @param mapper 格式映射函数
   */
  setFormatMapper(mapper: (format: string, factory: OutputAdapterFactory) => OutputAdapter | null): void {
   ...
   * @private
   */
  private resolveAdapter(adapter: OutputAdapter | (() => OutputAdapter)): OutputAdapter {
