...
export class ContextManager {
     ...
     * @returns 根上下文
     */
    createRootContext(document, options) {
        return {
            document,
            options,
            output: {},
     ...
     * @returns 子上下文
     */
    createChildContext(parentContext, pathElement, variables) {
        return {
            ...parentContext,
     ...
     * @returns 路径标识符
     */
    getPathIdentifier(context, separator = '/') {
     ...
     */
    getElementIdentifier(pathElement) {
        const matches = pathElement.match(/\[([^\d][^\]]*)\]/);
        if (matches && matches.length > 1) {
            return matches[1];
        }
     ...
     * @returns 变量值
     */
    getVariable(context, name, defaultValue, searchParent = true) {
        // 首先在当前上下文中查找
     ...
     */
    getParentResultByType(context, type) {
     ...
     */
    getParentResultByIndex(context, index) {
     ...
     * @returns 匹配的结果数组
     */
    getAllParentResultsByType(context, type) {
     ...
     * @returns 是否存在
     */
    hasParentResultOfType(context, type) {
     ...
     * @returns 父结果链
     */
    getParentResultsByPath(context) {
        const result = {};
        // 将路径和父结果组合
     ...
     * @returns 合并后的父结果链
     */
    mergeParentResults(parentResults1, parentResults2) {
     ...
     * @returns 嵌套子上下文
     */
    createNestedContext(parentContext, pathElement, variables) {
     ...
     */
    getAncestorContext(context, level) {
        if (level === 0) {
            return context;
        }
        if (!context.parent) {
     ...
     */
    getAncestorContextByPathElement(context, pathElement) {
        if (!context.parent) {
            return undefined;
        }
        // 检查父上下文的路径最后一个元素是否匹配
     ...
     */
    deepCloneNestedContext(context) {
        // 基础克隆，保持共享引用
        const cloned = {
            document: context.document,
            options: context.options,
        ...
        if (context.nested && context.nested.size > 0) {
            for (const [key, nestedContext] of context.nested.entries()) {
                ...
                cloned.nested.set(key, clonedNested);
            }
        }
        return cloned;
    }
