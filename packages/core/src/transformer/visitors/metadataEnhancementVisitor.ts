...
export class MetadataEnhancementVisitor extends BaseVisitor {
  ...
  private extractFromSemantics(
    ...
    for (const [key, value] of Object.entries(semantics)) {
      // 检查是否有映射
      const mappedKey =
        this.options.semanticsToMeta && this.options.semanticsToMeta[key];
   ...
   * @private
   */
  private extractFromContextVariables(
    context: TransformContext,
    meta: Record<string, any>
  ): void {
    ...
    // 遍历变量映射
    for (const [varName, metaKey] of Object.entries(
      this.options.variablesToMeta
    )) {
