...
export class ElementVisitor implements TransformerVisitor {
   ...
   * @param options 配置选项
   */
  constructor(priority: number = 20, options: ElementVisitorOptions = {}) {
  ...
  async visitElement(
    ...
    // 应用特殊元素处理器
    if (
      this.options.specialElements &&
      this.options.specialElements[element.tagName]
    ) {
      try {
  ...
  private processAttributes(element: Element): Element {
    ...
    // 转换数值属性
    if (
      this.options.convertNumericAttributes &&
      this.options.numericAttributes
    ) {
    ...
    // 转换布尔属性
    if (
      this.options.convertBooleanAttributes &&
      this.options.booleanAttributes
    ) {
    ...
    if (this.options.attributeConverters) {
      for (const [attrName, converter] of Object.entries(
        this.options.attributeConverters
      )) {
        if (
  ...
  private collectMetadata(element: Element): Element {
    if (
      !element.attributes ||
      !this.options.metadataAttributes ||
      this.options.metadataAttributes.length === 0
    ) {
      return element;
    }
