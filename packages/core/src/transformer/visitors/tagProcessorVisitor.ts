 ...
 * 标签处理访问者选项
 */
export interface TagProcessorVisitorOptions {
  /**
   * 是否忽略处理器错误
   ...
   * 默认为true
   */
  applyWildcardProcessors?: boolean;
}
 ...
 * 支持处理器链执行和通配符处理
 */
export class TagProcessorVisitor implements TransformerVisitor {
  /**
   * 访问者名称
   */
   ...
   * @param priority 优先级，默认为50
   * @param options 配置选项
   */
  constructor(
    registry: TagProcessorRegistry,
    priority: number = 50,
    options: TagProcessorVisitorOptions = {}
  ) {
