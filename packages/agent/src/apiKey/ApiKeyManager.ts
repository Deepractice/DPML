 ...
 * API提供商类型
 */
export type ApiProvider = 'openai' | 'anthropic' | 'azure' | 'mistral' | 'custom';
...
export class ApiKeyManager {
   ...
   * @returns 刷新后的API密钥
   */
  async refreshApiKey(envName: string, validate: boolean = false): Promise<string> {
    // 清除缓存
   ...
   */
  async getApiKeyWithFallback(envNames: string[], validate: boolean = false): Promise<string> {
    const errors: Error[] = [];
