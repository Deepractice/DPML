 ...
 * 文件系统存储选项
 */
export interface FileSystemAgentStateManagerOptions extends AgentStateManagerOptions {
  /** 状态文件存储目录 */
  storageDir: string;
...
export class FileSystemAgentStateManager implements AgentStateManager {
  ...
  async initialize(): Promise<boolean> {
    ...
    try {
      ...
      const stateFiles = files.filter(file => file.endsWith(this.fileExtension));
  ...
  async deleteSession(sessionId: string): Promise<boolean> {
    ...
    try {
      ...
      console.error(`Failed to delete state file for session ${sessionId}:`, error);
      return false;
    }
   ...
   * @param state 状态对象
   */
  private async saveStateToFile(sessionId: string, state: AgentState): Promise<void> {
    const filePath = this.getStateFilePath(sessionId);
   ...
   */
  private async loadStateFromFile(sessionId: string): Promise<AgentState | null> {
    const filePath = this.getStateFilePath(sessionId);
