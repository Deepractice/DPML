...
describe('文本摘要代理用例测试', () => {
          ...
          createConnector: vi.fn().mockImplementation(() => {
            return {
              complete: vi.fn().mockResolvedValue({
                content:
                  '文本摘要：人工智能(AI)是计算机科学分支，旨在创建模拟人类智能的系统。从1950年代发展至今，经历了符号主义到机器学习再到深度学习的演变。近期大型语言模型(如GPT)引起广泛关注。AI在自然语言处理、计算机视觉等多领域取得突破，但同时面临伦理、偏见等挑战。研究者正致力于开发更透明、公平、安全的AI系统。',
                usage: {
                  promptTokens: 300,
              ...
              completeStream: vi.fn().mockImplementation(async function* () {
                yield {
                  content: '文本摘要：人工智能(AI)是计算机科学分支，',
                  done: false,
                };
                yield {
                  content: '旨在创建模拟人类智能的系统。从1950年代发展至今，',
                  done: false,
                };
                yield {
                  content: '经历了符号主义到机器学习再到深度学习的演变。',
                  done: false,
                };
                yield {
                  content: '近期大型语言模型(如GPT)引起广泛关注。',
                  done: false,
                };
                yield {
                  content: 'AI在自然语言处理、计算机视觉等多领域取得突破，',
                  done: false,
                };
                yield { content: '但同时面临伦理、偏见等挑战。', done: false };
                yield {
                  content: '研究者正致力于开发更透明、公平、安全的AI系统。',
                  done: true,
                };
              }),
            };
  ...
  it('应能保留原文中的核心观点和关键信息', async () => {
    ...
    expect(containedKeypoints).toBeGreaterThanOrEqual(
      Math.floor(keypoints.length * 0.6)
    );
  });
});
