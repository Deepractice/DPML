...
describe('编程助手代理用例测试', () => {
  ...
  beforeEach(async () => {
          ...
          createConnector: vi.fn().mockImplementation(() => {
            return {
              complete: vi.fn().mockResolvedValue({
                content: '以下是使用TypeScript实现二分查找算法的代码：\n\n```typescript\nfunction binarySearch(arr: number[], target: number): number {\n  let left = 0;\n  let right = arr.length - 1;\n  \n  while (left <= right) {\n    const mid = Math.floor((left + right) / 2);\n    \n    if (arr[mid] === target) {\n      return mid;\n    }\n    \n    if (arr[mid] < target) {\n      left = mid + 1;\n    } else {\n      right = mid - 1;\n    }\n  }\n  \n  return -1; // 目标值不在数组中\n}\n```\n\n这个实现的时间复杂度是O(log n)，因为每次比较都会将搜索范围减半。',
                usage: { promptTokens: 120, completionTokens: 200, totalTokens: 320 }
              }),
              completeStream: vi.fn().mockImplementation(async function* () {
                yield { content: '以下是使用TypeScript实现二分查找算法的代码：\n\n```typescript\n', done: false };
                yield { content: 'function binarySearch(arr: number[], target: number): number {\n  let left = 0;\n  let right = arr.length - 1;\n  \n  while (left <= right) {\n    const mid = Math.floor((left + right) / 2);\n    \n    if (arr[mid] === target) {\n      return mid;\n    }\n    \n    if (arr[mid] < target) {\n      left = mid + 1;\n    } else {\n      right = mid - 1;\n    }\n  }\n  \n  return -1; // 目标值不在数组中\n}\n```\n\n', done: false };
                yield { content: '这个实现的时间复杂度是O(log n)，因为每次比较都会将搜索范围减半。', done: true };
              })
            };
          })
        }
    ...
    // 读取编程助手代理定义文件
    const agentFilePath = path.join(__dirname, '../fixtures/programming-assistant.xml');
  ...
  <prompt>
    ...
    const agentConfig = {
      ...
      executionConfig: {
        ...
        apiType: 'openai',
        systemPrompt: '你是一个专业的编程助手，擅长编写高质量代码和解决编程问题。'
      }
    };
