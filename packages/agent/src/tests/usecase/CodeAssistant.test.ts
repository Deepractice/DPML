...
console.log("总价: " + total);
  ...
  beforeEach(async () => {
            ...
            return {
              complete: vi.fn().mockImplementation((prompt) => {
                ...
                if (prompt.includes('解释') || prompt.includes('explanation')) {
                  ...
                  return Promise.resolve({
                    content: '我是代码助手，我可以帮助你解释代码、优化代码、调试问题或生成新代码。请告诉我你需要什么样的帮助。',
                    usage: { promptTokens: 50, completionTokens: 30, totalTokens: 80 }
                  });
                }
              }),
              completeStream: vi.fn().mockImplementation(async function* (prompt) {
                if (prompt.includes('解释') || prompt.includes('explanation')) {
                  yield { content: '这段代码实现了斐波那契数列的递归计算。', done: false };
                  yield { content: '斐波那契数列是一个以递归方式定义的数列，其中每个数都是前两个数的和，前两个数为0和1。', done: false };
                  yield { content: '\n\n函数`fibonacci(n)`接受一个参数n表示要计算的斐波那契数列中的第n个数。', done: false };
                  yield { content: '如果n小于等于1，则直接返回n；', done: false };
                  yield { content: '否则，函数会递归调用自身计算f(n-1)和f(n-2)，并返回它们的和。', done: false };
                  yield { content: '\n\n这个实现方式简单直观，但时间复杂度为O(2^n)，效率较低，因为存在大量重复计算。', done: true };
                } else if (prompt.includes('优化') || prompt.includes('optimize')) {
                  yield { content: '以下是优化后的斐波那契数列计算函数：\n\n```javascript', done: false };
                  yield { content: '\nfunction fibonacci(n) {', done: false };
                  yield { content: '\n  if (n <= 1) return n;', done: false };
                  yield { content: '\n  ', done: false };
                  yield { content: '\n  let a = 0, b = 1;', done: false };
                  yield { content: '\n  for (let i = 2; i <= n; i++) {', done: false };
                  ...
                  yield { content: '\n  return b;', done: false };
                  yield { content: '\n}\n```', done: false };
                  yield { content: '\n\n这个迭代版本的时间复杂度为O(n)，空间复杂度为O(1)，比递归版本效率高得多。', done: false };
                  yield { content: '通过使用两个变量a和b来存储当前计算所需的前两个斐波那契数，避免了重复计算。', done: true };
                } else if (prompt.includes('错误') || prompt.includes('debug')) {
                  yield { content: '这段代码有以下错误：\n\n', done: false };
                  yield { content: '1. 循环条件错误：`for (let i = 0; i <= items.length; i++)` 中的条件应该是 `i < items.length`，', done: false };
                  yield { content: '否则会导致访问数组越界。\n\n', done: false };
                  yield { content: '2. 在数组长度为3的情况下，当i=3时，`items[i]`会尝试访问数组的第4个元素（索引为3），', done: false };
                  yield { content: '但该元素不存在，因此`items[i].price`会产生"Cannot read property \'price\' of undefined"错误。\n\n', done: false };
                  yield { content: '修复后的代码：\n\n```javascript\n', done: false };
                  yield { content: 'function calculateTotal(items) {\n', done: false };
                  yield { content: '  let total = 0;\n', done: false };
                  yield { content: '  for (let i = 0; i < items.length; i++) {\n', done: false };
                  yield { content: '    total += items[i].price;\n', done: false };
                  yield { content: '  }\n', done: false };
                  yield { content: '  return total;\n', done: false };
                  yield { content: '}\n```', done: true };
                } else {
                  yield { content: '我是代码助手，我可以帮助你解释代码、优化代码、调试问题或生成新代码。', done: false };
                  yield { content: '请告诉我你需要什么样的帮助。', done: true };
                }
              })
            };
          })
    ...
    // 读取代码助手代理定义文件
    const agentFilePath = path.join(__dirname, '../fixtures/code-assistant.xml');
  ...
  it('应能提供代码解释', async () => {
    ...
    keyTerms.forEach(term => {
      expect(response.response?.text.toLowerCase()).toContain(term.toLowerCase());
    });
  });
