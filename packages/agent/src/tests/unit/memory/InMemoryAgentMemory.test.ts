...
import type { Memory, MemoryItem } from '../../../memory';
...
describe('InMemoryAgentMemory', () => {
  ...
  describe('memory operations', () => {
    ...
    it('should store and retrieve memory correctly (UT-MEM-001)', async () => {
      ...
      expect((retrievedMemory.content as MemoryItem[])[0].text).toBe(
        'Hello, agent!'
      );
    ...
    it('should properly manage memory for multiple sessions (UT-MEM-002)', async () => {
      ...
      expect((memory1.content as MemoryItem[])[0].text).toBe(
        'Message from session 1'
      );
      expect((memory2.content as MemoryItem[])[0].text).toBe(
    ...
    it('should clear memory correctly (UT-MEM-004)', async () => {
      ...
      const testMemory: Memory = {
        id: testSessionId,
        content: [
          { text: 'Test message', role: 'user', timestamp: Date.now() },
        ] as MemoryItem[],
      };
  ...
  describe('memory compression', () => {
    it('should compress memory when exceeding max items (UT-MEM-008, UT-MEM-010)', async () => {
      ...
      // 验证最近的消息被保留
      expect((retrievedMemory.content as MemoryItem[])[4].text).toBe(
        'Response 3'
      );
    });
    ...
    it('should prioritize important memory items (UT-MEM-009)', async () => {
      ...
      // 验证所有system消息都被保留
      const systemCount = (retrievedMemory.content as MemoryItem[]).filter(
      ...
      // 验证最近的非system消息被保留
      expect((retrievedMemory.content as MemoryItem[])[2].text).toBe(
        'Message 2'
      );
    });
  });
