...
import { AgentStatus, InMemoryAgentStateManager } from '../../../state';
...
describe('InMemoryAgentStateManager', () => {
  ...
  describe('state transitions', () => {
    ...
    it('should validate state transitions', () => {
      // 有效转换
      expect(
        stateManager.isValidTransition(AgentStatus.IDLE, AgentStatus.THINKING)
      ).toBe(true);
      expect(
        stateManager.isValidTransition(
      ...
      // 无效转换
      expect(
        stateManager.isValidTransition(AgentStatus.IDLE, AgentStatus.DONE)
      ).toBe(false);
      expect(
        stateManager.isValidTransition(AgentStatus.ERROR, AgentStatus.EXECUTING)
      ).toBe(false);
    });
  ...
  describe('serialization', () => {
    ...
    it('should throw error when serializing non-existent session', async () => {
      await expect(stateManager.serializeState('non-existent')).rejects.toThrow(
        /does not exist/
      );
    });
    ...
    it('should deserialize state from JSON string', async () => {
      ...
      const state = await stateManager.deserializeState(
        'new-session',
        serialized
      );
