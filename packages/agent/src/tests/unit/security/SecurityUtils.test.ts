...
import { SecurityUtils, SecurityConstants } from '../../../security';
...
describe('SecurityUtils', () => {
  ...
  describe('safeGetEnv', () => {
    ...
    it('should safely get environment variables (UT-SEC-001)', () => {
      ...
      expect(SecurityUtils.safeGetEnv('NON_EXISTENT_VAR', 'default')).toBe(
        'default'
      );
    });
  });
  ...
  describe('isSafeFileExtension', () => {
    ...
    it('should support custom safe extensions', () => {
      ...
      expect(
        SecurityUtils.isSafeFileExtension('file.xml', customExtensions)
      ).toBe(true);
      expect(
        SecurityUtils.isSafeFileExtension('file.yaml', customExtensions)
      ).toBe(true);
      expect(
        SecurityUtils.isSafeFileExtension('file.txt', customExtensions)
      ).toBe(false);
    });
  ...
  describe('SecurityConstants', () => {
    it('should define common sensitive patterns', () => {
      expect(SecurityConstants.SENSITIVE_ENV_PATTERNS.length).toBeGreaterThan(
        0
      );
