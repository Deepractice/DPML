    ...
    if (event === 'unhandledRejection') {
      ...
      if (
        ...
        (error.message === 'AbortError' ||
          ...
          (error instanceof LLMConnectorError &&
            error.type === LLMErrorType.ABORTED))
      ) {
        // 忽略中断相关的异常
        return true;
      }
...
describe('AbstractLLMConnector', () => {
  ...
  describe('complete方法', () => {
    ...
    it('应处理重试逻辑', async () => {
      ...
      executeSpy.mockImplementationOnce(() => {
        throw new LLMConnectorError('Rate limit', LLMErrorType.RATE_LIMIT, {
          retryable: true,
        });
      });

      executeSpy.mockImplementationOnce(() => {
        throw new LLMConnectorError('Server error', LLMErrorType.SERVER, {
          retryable: true,
        });
      });
  ...
  describe('completeStream方法', () => {
    ...
    it('应处理中止请求', async () => {
      ...
      vi.spyOn(
        connector as any,
        'executeCompletionStream'
      ).mockImplementationOnce(async function* () {
        throw new Error('AbortError');
      });

      try {
        const asyncIterator = connector
  ...
  describe('abortRequest方法', () => {
    it('应中止指定请求', async () => {
      ...
      // 将模拟的Map应用到connector
      vi.spyOn(connector as any, 'activeRequests', 'get').mockReturnValue(
        mockActiveRequests
      );
  ...
  describe('错误处理', () => {
    ...
    it('应正确识别可重试的错误类型', () => {
      // 可重试错误类型
      expect(
        (connector as any).isRetryableError(
      ...
      expect(
        (connector as any).isRetryableError(
          new LLMConnectorError('Custom', LLMErrorType.UNKNOWN, {
            retryable: true,
          })
        )
      ).toBe(true);
      ...
      expect(
        (connector as any).isRetryableError(
          new LLMConnectorError('Custom', LLMErrorType.TIMEOUT, {
            retryable: false,
          })
        )
      ).toBe(false);
