...
describe('AgentTagProcessor', () => {
  ...
  it('UT-AP-001: 应正确处理基本属性(id, version)', async () => {
    ...
    const resultWithoutVersion = await processor.process(
      elementWithoutVersion,
      context
    );
  ...
  it('UT-AP-002: 应正确收集和验证子标签', async () => {
    ...
    // 创建有完整子标签的agent元素
    const completeElement = createAgentElement({ id: 'complete-agent' }, [
      llmElement,
      promptElement,
    ]);
    ...
    // 处理不完整元素
    const incompleteResult = await processor.process(
      incompleteElement,
      context
    );
    ...
    expect(incompleteResult.metadata?.validationErrors?.[0].code).toBe(
