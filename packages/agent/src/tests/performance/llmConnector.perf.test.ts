...
import { AbstractLLMConnector } from '../../connector/AbstractLLMConnector';
import { LLMConnectorError, LLMErrorType } from '../../connector/LLMConnector';
  ...
  protected async *executeCompletionStream(
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      await new Promise(resolve =>
        setTimeout(resolve, this.delay / chunks.length)
      );
...
describe('LLM连接器性能测试', () => {
  ...
  beforeEach(() => {
    ...
    // 设置模拟错误
    connector.setMockError(
      JSON.stringify([{ role: 'user', content: '触发错误' }]),
  ...
  test('重试机制能够处理临时错误', async () => {
    // 创建一个会先失败然后成功的响应
    const requestKey = JSON.stringify([{ role: 'user', content: '重试测试' }]);
  ...
  test('多个相同请求同时发起只会执行一次API调用并共享结果', async () => {
    ...
    const results = await Promise.all(
      Array(5)
        .fill(null)
        .map(() => connector.complete(options))
    );
    const totalTime = Date.now() - startTime;
