...
export abstract class AbstractLLMConnector implements LLMConnector {
  ...
  private async completeWithRetry(
    ...
    try {
      ...
      throw (
        lastError ||
        new LLMConnectorError('达到最大重试次数', LLMErrorType.UNKNOWN)
      );
    } finally {
      // 清理请求
      ...
      try {
        while (attempt <= retry.maxRetries) {
          try {
            ...
            // 检查错误是否可重试
            if (
              !this.isRetryableError(llmError) ||
  ...
  protected normalizeError(error: unknown): LLMConnectorError {
    ...
    if (error instanceof Error) {
      ...
      if (
        ...
        return new LLMConnectorError('请求被中断', LLMErrorType.ABORTED, {
          originalError: error,
          retryable: false,
        });
      }
      ...
      if (error.name === 'TimeoutError' || error.message.includes('timeout')) {
        return new LLMConnectorError('请求超时', LLMErrorType.TIMEOUT, {
          originalError: error,
          retryable: true,
        });
      }
      ...
      // 通用错误
      return new LLMConnectorError(error.message, LLMErrorType.UNKNOWN, {
        originalError: error,
        retryable: false,
      });
    }
