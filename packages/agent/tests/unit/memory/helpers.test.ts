...
describe('Memory Helpers', () => {
  ...
  describe('storeAssistantMessage', () => {
    it('should add assistant message to empty memory', async () => {
      // 存储助手消息
      await storeAssistantMessage(memoryManager, testSessionId, 'I can help you with that!');
    ...
    it('should add assistant message after user message', async () => {
      ...
      // 再存储助手消息
      await storeAssistantMessage(memoryManager, testSessionId, 'Yes, I can help you!');
  ...
  describe('buildConversationContext', () => {
    it('should build context with only system prompt for empty memory (UT-MEM-007)', async () => {
      ...
      const context = await buildConversationContext(memoryManager, testSessionId, systemPrompt);
