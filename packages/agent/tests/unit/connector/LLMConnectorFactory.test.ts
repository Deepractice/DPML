...
describe('LLMConnectorFactory', () => {
  ...
  beforeEach(() => {
    ...
    // 模拟连接器构造函数
    vi.mock('../../../src/connector/providers/OpenAIConnector', () => ({
    ...
    vi.mock('../../../src/connector/providers/AnthropicConnector', () => ({
      AnthropicConnector: vi.fn().mockImplementation((apiKey, apiUrl, apiVersion) => ({
        getType: () => 'anthropic',
        apiKey,
        apiUrl,
        apiVersion
  ...
  describe('createConnector方法', () => {
    it('应创建OpenAI连接器', () => {
      ...
      expect(OpenAIConnector).toHaveBeenCalledWith('test-openai-key', 'https://api.openai.com/v1');
    });
    ...
    it('应对不支持的API类型抛出错误', () => {
      ...
      expect(() => LLMConnectorFactory.createConnector(config)).toThrow(LLMConnectorError);
      try {
    ...
    it('应在环境变量缺失时抛出错误', () => {
      ...
      try {
        ...
        expect((error as LLMConnectorError).type).toBe(LLMErrorType.AUTHENTICATION);
        expect((error as LLMConnectorError).message).toContain('OPENAI_API_KEY');
      }
    });
    ...
    it('应支持无需认证的本地模型', () => {
      ...
      expect(connector).toBeDefined();
      expect(connector.getType()).toBe('openai');
      expect(OpenAIConnector).toHaveBeenCalledWith('', 'http://localhost:1234/v1');
    });
