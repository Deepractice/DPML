...
import { AgentImpl } from '../../../src/agent/AgentImpl';
import { AgentStatus } from '../../../src/state/AgentState';
import { CompletionResult } from '../../../src/connector/LLMConnector';
...
describe('AgentImpl', () => {
  ...
  beforeEach(() => {
    ...
    mockMemory.retrieve.mockResolvedValue({
      ...
      content: [
        ...
        { text: '你好！有什么可以帮助你的吗？', role: 'assistant', timestamp: Date.now() - 500 }
      ]
    });
  ...
  describe('执行功能', () => {
    it('应正确执行同步请求', async () => {
      ...
      // 验证状态更新
      expect(mockStateManager.updateState).toHaveBeenCalledWith('test-session', expect.objectContaining({
      ...
      expect(mockStateManager.addMessage).toHaveBeenCalledWith('test-session', expect.objectContaining({
        role: 'user',
        content: '测试查询'
      }));
      expect(mockStateManager.addMessage).toHaveBeenCalledWith('test-session', expect.objectContaining({
      ...
      // 验证事件触发
      expect(mockEventSystem.emit).toHaveBeenCalledWith('agent:thinking', expect.anything());
      expect(mockEventSystem.emit).toHaveBeenCalledWith('agent:responding', expect.anything());
      expect(mockEventSystem.emit).toHaveBeenCalledWith('agent:llm:before', expect.anything());
      expect(mockEventSystem.emit).toHaveBeenCalledWith('agent:llm:success', expect.anything());
      expect(mockEventSystem.emit).toHaveBeenCalledWith('agent:done', expect.anything());
    ...
    it('应处理执行错误', async () => {
      ...
      // 验证错误事件
      expect(mockEventSystem.emit).toHaveBeenCalledWith('agent:error', expect.anything());
    ...
    it('应正确中断执行', async () => {
      ...
      // 验证中断事件
      expect(mockEventSystem.emit).toHaveBeenCalledWith('agent:interrupted', expect.anything());
    });
    ...
    it('应正确重置状态', async () => {
      ...
      // 验证重置事件
      expect(mockEventSystem.emit).toHaveBeenCalledWith('agent:reset', expect.anything());
    });
  });
}); 