...
import { AgentRequest } from '../../src/agent/types';
...
describe('代理执行性能测试', () => {
  ...
  test('并发执行能够正确控制并发数', async () => {
    ...
    const requests: AgentRequest[] = Array(requestCount).fill(null).map((_, i) => ({
      text: `测试请求 ${i}`,
      sessionId: `session-${i}`
    }));
    ...
    const results = await Promise.all(
      requests.map(req => agent.execute(req))
    );
  ...
  test('流式执行性能', async () => {
    ...
    mockConnector.completeStream.mockImplementation(() => mockStreamGenerator());
    ...
    // 收集所有块
    const chunks: string[] = [];
    for await (const chunk of agent.executeStream({ text: '流测试', sessionId: 'stream-test' })) {
