...
describe('客服代理用例测试', () => {
  ...
  beforeEach(async () => {
          ...
          createConnector: vi.fn().mockImplementation(() => {
            return {
              complete: vi.fn().mockResolvedValue({
                content: '您好！很抱歉听到您的订单有延迟。我可以帮您查询订单状态，请提供您的订单号。通常情况下，标准配送需要3-5个工作日，但特殊情况如天气或供应链问题可能导致延迟。我们会尽快处理您的订单，感谢您的耐心等待。',
                usage: { promptTokens: 80, completionTokens: 120, totalTokens: 200 }
              }),
              completeStream: vi.fn().mockImplementation(async function* () {
                yield { content: '您好！很抱歉听到您的订单有延迟。', done: false };
                yield { content: '我可以帮您查询订单状态，请提供您的订单号。', done: false };
                yield { content: '通常情况下，标准配送需要3-5个工作日，但特殊情况如天气或供应链问题可能导致延迟。', done: false };
                yield { content: '我们会尽快处理您的订单，感谢您的耐心等待。', done: true };
              })
            };
          })
        }
    ...
    // 读取客服代理定义文件
    const agentFilePath = path.join(__dirname, '../fixtures/customer-service.xml');
