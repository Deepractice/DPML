...
describe('研究助手代理用例测试', () => {
  ...
  beforeEach(async () => {
          ...
          createConnector: vi.fn().mockImplementation(() => {
            return {
              complete: vi.fn().mockResolvedValue({
                content: '根据最新研究，量子计算在解决复杂优化问题方面显示出巨大潜力。2023年的研究表明，量子算法在特定类别的优化问题上可以实现指数级加速。',
                usage: { promptTokens: 100, completionTokens: 50, totalTokens: 150 }
              }),
              completeStream: vi.fn().mockImplementation(async function* () {
                yield { content: '根据最新研究', done: false };
                yield { content: '，量子计算在解决复杂优化问题方面显示出巨大潜力', done: false };
                yield { content: '。2023年的研究表明，量子算法在特定类别的优化问题上可以实现指数级加速。', done: true };
              })
            };
          })
        }
    ...
    // 读取研究助手代理定义文件
    const agentFilePath = path.join(__dirname, '../fixtures/research-assistant.xml');
