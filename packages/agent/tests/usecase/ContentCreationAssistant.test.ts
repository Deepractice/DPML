...
describe('内容创作助手代理用例测试', () => {
  ...
  // 内容主题请求
  const contentTopicRequest = '请为一篇关于可持续发展与科技创新的文章提供5个标题建议';
  ...
  beforeEach(async () => {
          ...
          createConnector: vi.fn().mockImplementation(() => {
            return {
              ...
              completeStream: vi.fn().mockImplementation(async function* (prompt) {
                if (prompt.includes('润色') || prompt.includes('polish')) {
                  yield { content: '润色版：\n\n', done: false };
                  yield { content: '人工智能正在深刻变革我们的工作模式。', done: false };
                  yield { content: '随着技术的不断发展，许多传统职业将被自动化系统取代，', done: false };
                  yield { content: '但这一转变同时也将催生大量前所未有的就业机会。', done: false };
                  yield { content: '在这个快速变化的时代，我们需要主动适应这些转变，', done: false };
                  yield { content: '持续学习和掌握新技能，才能在AI驱动的未来保持竞争力。', done: true };
                } else if (prompt.includes('标题') || prompt.includes('title')) {
                  yield { content: '以下是关于可持续发展与科技创新的文章标题建议：\n\n', done: false };
                  yield { content: '1. 《科技创新：驱动可持续发展的关键引擎》\n', done: false };
                  yield { content: '2. 《绿色科技：连接现在与可持续未来的桥梁》\n', done: false };
                  yield { content: '3. 《数字化转型与环保：寻找可持续发展的平衡点》\n', done: false };
                  yield { content: '4. 《智能技术如何重塑可持续城市生态》\n', done: false };
                  yield { content: '5. 《从创新到实践：科技赋能的可持续发展之路》', done: true };
                } else if (prompt.includes('灵感') || prompt.includes('inspiration')) {
                  yield { content: '关于人工智能与工作的未来，您可以考虑以下几个角度展开：\n\n', done: false };
                  yield { content: '1. **技能转换的必要性**：讨论人们如何从传统技能向数字技能转变\n', done: false };
                  yield { content: '2. **终身学习的重要性**：在AI时代，持续学习成为职业发展的必要条件\n', done: false };
                  yield { content: '3. **人机协作的新模式**：探讨AI不是替代人类，而是增强人类能力的工具\n', done: false };
                  yield { content: '4. **创造力与情感智能的价值**：这些难以被AI复制的人类特质将变得更加珍贵\n', done: false };
                  yield { content: '5. **教育体系的转型**：如何调整教育以适应AI驱动的工作环境\n', done: false };
                  yield { content: '6. **社会安全网的重要性**：如何保护那些因技术变革而失业的人群', done: true };
                } else {
                  yield { content: '我是内容创作助手，可以帮助您润色文章、提供写作灵感、建议标题和改进内容结构。', done: false };
                  yield { content: '请告诉我您需要什么样的创作帮助。', done: true };
                }
              })
            };
          })
    ...
    // 读取内容创作助手代理定义文件
    const agentFilePath = path.join(__dirname, '../fixtures/content-creation-assistant.xml');
  ...
  <prompt>
    ...
    const agentConfig = {
      ...
      executionConfig: {
        defaultModel: 'gpt-4',
        apiType: 'openai',
        systemPrompt: '你是一个专业的内容创作助手，擅长为各类内容提供创意支持和改进建议。'
      }
    };
