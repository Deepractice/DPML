...
describe('CLI Command Parsing Issues', () => {
  ...
  describe('Issue 1: --list选项要求提供domain参数', () => {
    ...
    test('list作为子命令的测试（如果实现策略改变可以启用）', async () => {
      const result = await runCLIProcess(['list']);

      // 我们当前没有实现list子命令，而是使用--list选项
      // 这个测试只是记录可能的替代实现方式，修改测试期望值
      expect(result.exitCode).toBe(1); // 预期失败，因为我们不支持list子命令
      expect(result.stderr).toContain('Domain not found'); // 而是将其视为无效域名
    });
  });
  ...
  describe('整体命令行用户体验', () => {
    ...
    test('不带参数执行应显示帮助信息', async () => {
      ...
      expect(result.exitCode).toBe(0); // 修改期望值，现在显示帮助而不是错误
