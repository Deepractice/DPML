...
export async function writeJsonFile<T = any>(
  ...
  try {
    ...
    throw new Error(
      `写入JSON文件失败: ${filePath} - ${(error as Error).message}`
    );
  }
}
...
export async function copyFile(
  ...
  try {
    ...
    throw new Error(
      `复制文件失败: ${sourcePath} -> ${destPath} - ${(error as Error).message}`
    );
  }
}
...
export async function removeFile(filePath: string): Promise<void> {
  try {
    ...
    if ((error as NodeJS.ErrnoException).code !== 'ENOENT') {
      throw new Error(
        `删除文件失败: ${filePath} - ${(error as Error).message}`
      );
    }
  }
}
...
export async function removeTempDirectory(dirPath: string): Promise<void> {
  try {
    ...
    throw new Error(
      `删除临时目录失败: ${dirPath} - ${(error as Error).message}`
    );
  }
}
...
export async function atomicWriteFile(
  ...
  try {
    ...
    throw new Error(
      `原子写入文件失败: ${filePath} - ${(error as Error).message}`
    );
  }
}
...
export async function getFileStats(filePath: string): Promise<fs.Stats> {
  try {
    ...
    throw new Error(
      `获取文件状态失败: ${filePath} - ${(error as Error).message}`
    );
  }
}
