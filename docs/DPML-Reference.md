...
| 属性 | 适用范围 | 格式 | 说明 |
|------|---------|------|------|
| **id** | 所有标签 | 字符串 | 提供标签的唯一标识符，便于引用。命名规则：字母开头，允许字母、数字、下划线、连字符。 |
| **version** | 根标签 | 主版本.次版本 | 指示DPML规范版本，影响解析规则。如"1.0"、"2.1"。 |
| **lang** | 根标签 | ISO语言代码 | 指定语言，影响内容解释。如"zh-CN"、"en-US"。 |

### 3.2 继承属性

| 属性 | 适用范围 | 格式 | 说明 |
|------|---------|------|------|
| **extends** | 所有标签 | URI或ID引用 | 从其他标签继承定义，实现复用。可引用本地文件、远程文件或文档内ID。 |

### 3.3 控制属性

| 属性 | 适用范围 | 格式 | 说明 |
|------|---------|------|------|
| **schema** | 根标签 | URI | 提供验证规则来源，指向schema定义。 |
| **mode** | 根标签 | 枚举值 | 控制解析器严格程度。可选值："strict"、"loose"(默认)。 |
