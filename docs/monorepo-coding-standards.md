...
为避免因目录问题导致命令执行错误，我们提供了项目级命令执行助手：

1. **使用项目命令执行助手**
   ```bash
   # 在项目根目录执行命令
   pnpm run run . git status
   
   # 在特定包目录执行命令
   pnpm run run prompt test
   pnpm run run core build
   ```

2. **助手功能**
   - 自动定位到正确的目录（项目根目录或特定包目录）
   - 执行前显示当前工作目录，防止错误
   - 支持任何命令，包括git、pnpm等
   - 无需修改系统全局配置
...
3. **最佳实践**
   - 对于跨包操作，始终使用命令执行助手
   - 对于涉及文件路径的命令，使用相对于特定包或项目根目录的路径
   - 在CI/CD环境中，推荐使用完整路径或命令执行助手
